...
static void usb_device_midi_rx_process(uint8_t itf);
...
{
    usb_device_midi_rx_process(itf);
}
...
static void usb_device_midi_rx_process(uint8_t itf)
{
    ...
    /* Leave unread packets in TinyUSB's FIFO when the application queue is
     * full.  This is the transport backpressure boundary: never drain and
     * silently discard MIDI under control-side load. */
    while ((midi_usb_rx_free_packets() != 0U)
           && (processed < USB_DEVICE_MIDI_RX_BUDGET)
           && tud_midi_n_packet_read(itf, packet)) {
