...
Le bleu des tracks sélectionnables venait de `Drivers/Drv_app/Src/led_rgb.c:led_apply_track_select_hall_scene()`, qui testait `ui_get_track_family(hall) != UI_TRACK_FAMILY_OFF` et appliquait `LED_FIXED_DARK_BLUE_*`; le track actif était ensuite forcé en blanc. En conséquence, `Looper` et les Inputs étaient bleus, tandis que `Master` et `FX`, configurés UI en `Off/Audio`, étaient noirs. La scène consulte maintenant `track_topology_is_special()` avant le chemin famille : tous les Specials sont violets, y compris le Special actif, tandis que le rendu Play reste inchangé. Le violet retenu est `LED_FIXED_VIOLET_* = (128, 0, 128)`.
...
| Specials bleus/noirs/blancs | Corrigé : `led_apply_track_select_hall_scene()` classe d’abord chaque hall par `track_topology_is_special()`; le chemin Special applique `LED_FIXED_VIOLET_*` avant toute règle d’activation. | `Drivers/Drv_app/Src/led_rgb.c`. | Violet fixe `(128, 0, 128)` pour Input, Looper, FX et Master; rendu Play et slots absents inchangés. |
| CFG Sampler/External dans le mauvais ordre | L’encodeur parcourt directement les valeurs enum et les labels indexés; valeurs persistées et affichage sont confondus. | `Src/UI/ui_param.c`, `Src/UI/ui_track_catalog.c`, `Src/Param/param_registry_catalog.c`. | Ajouter un ordre d’affichage indépendant et des conversions; ne pas renuméroter `ui_track_family_t` ni les valeurs Pattern/Project. |
| Anciennes sauvegardes rejetées après réindexation | `pattern_live_apply_snapshot()` et `pattern_live_seq_block_validate_plock_budget()` comparent identité au même index; ensuite config, sound, mix, sequence et matrices sont copiés par index. Project garde en plus `multi[track]` et les macro locks avec `track` brut. | `Src/Storage/pattern_live_ram.c`, `Src/Storage/project_v1.c`. | Avant validation/apply, normaliser en mémoire les tableaux par `track_topology_identity_t`; remapper aussi `seq.special`, sound/mix, routes, globals track-indexés, note-FX, Project multi et macro locks. Ne changer aucun format/version tant que cette migration reste dans les structures existantes. |
...
2. **Migration en mémoire.** Introduire une routine privée de remapping par identité pour Pattern/Project avant leurs validations actuelles. La routine doit être bijective pour les tracks actives, laisser les slots inutilisés LowCost hors payload logique et ne pas toucher aux formats Pattern v4, Project v4 ou Patch v3.
...
- Ajouter des fixtures Pattern/Project capturées avec l’ordre actuel puis chargées avec l’ordre cible; vérifier conservation des actions Special, MacroFX, globals Master, routes Looper, note-FX et macro locks.
...
- Vérifier explicitement la cohérence des versions Pattern/Project/Patch et que la taille/les offsets des structures persistées n’ont pas changé.
...
- Stockage/copie : `Inc/Storage/pattern_live_ram.h`, `Src/Storage/pattern_live_ram.c`, `Inc/Storage/project_v1.h`, `Src/Storage/project_v1.c`, `Src/Core/track_snapshot.c`, `Src/UI/ui_core_clipboard.c`, `Src/Storage/undo_v2.c`.
