...
déterminer quels paramètres Looper doivent réellement être p-lockables dans le produit courant ;

définir une seule allowlist ;

utiliser cette allowlist dans toutes les directions du mapping ;

supprimer les exclusions dupliquées et contradictoires.
...
vérifier track < SEQ_TRACK_COUNT ;

vérifier le set ;

vérifier la capacité compacte du set ;

vérifier le slot TONE selon le moteur actif lorsque nécessaire ;

calculer offset[set_id] + compact_slot ;

retourner NULL si le couple est invalide.
...
le nouveau firmware utilise directement 94 slots compacts par piste ;

aucun ancien grand tableau ou mapping RAM n’est conservé ;

aucun code de compatibilité historique n’est ajouté ;

le nouveau format de stockage est explicite et rejette l’ancien ;

les p-locks du firmware courant fonctionnent correctement ;

Undo/Redo et Clipboard utilisent la nouvelle identité compacte ;

les changements de moteur ne réutilisent aucun état invalide ;

les limites musicales restent inchangées ;

aucune allocation dynamique n’est ajoutée ;

le gain RAM_D2 Low-Cost est d’au moins 74 900 octets, confirmé par le linker ;

chaque passe validée produit un commit local précis, sans push ;

les builds requis sont Release Low-Cost et Release Premium uniquement.
