...
Les champs des structures effectivement publiees utilisent des codes scalaires
a largeur fixe; leur taille et leurs offsets structurants sont proteges par des
`_Static_assert` dans les headers de contract.
...
|---|---|---|---|---|---|
| Sample RAM | M4 loader -> M7 voices | payload dans le page pool; registry non-cacheable `AUDIO_SHARED_REGISTRY_SDRAM`; map ID dans `D2_IPC` | `global_slot`, `ram_slot`, generation et `{region, offset, length}` | payload clean, descriptor immutable, DMB, map `global_slot -> ram_slot` | stop PROGRAM/PARAM, fin des leases lecteurs, withdraw generation, puis pages libres |
...
STREAM conserve sa politique de leases physiques et son scheduler. Recorder,
...
insertions ordinaires; le protocole de pages et de leases ne change pas.
