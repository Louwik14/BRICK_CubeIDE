...
- Le bouton `POWER_BUTTON_SENSE` (PB5, actif haut) alimente le montage pendant l'appui; `POWER_HOLD` (PD4, push-pull sans pull) prend ensuite le relais a l'etat haut.
- Un appui runtime haut continu de 2000 ms arrete l'audio, eteint l'OLED, force `HOST_EN` PC7 haut via le gestionnaire de role USB (AP2161 enable actif bas), puis force immediatement PD4 bas. D5 maintient le boost tant que le bouton reste appuye; au relachement, R575 abaisse `BOOST_EN`, les rails tombent et le prochain appui repart d'un reset d'alimentation. Aucun service SD, cache, preview ou recorder n'est attendu.
- Le backend Premium ne demande ni ne simule de coupure materielle.
