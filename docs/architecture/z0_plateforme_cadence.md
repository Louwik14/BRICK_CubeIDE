...
## Addendum 2026-07-20 - preparation Z0 dual-core mono-coeur

- Z0 expose maintenant des entry points de domaine dans `brick6_app_init`: `brick6_platform_shared_init()`, `brick6_audio_domain_init()`, `brick6_system_domain_init()`, `brick6_audio_domain_start()`, `brick6_system_domain_start()` et `brick6_system_domain_process()`. `brick6_app_init()` et `brick6_app_process()` restent des wrappers de compat mono-coeur.
- Ordre de boot obtenu: `main()` conserve MPU/cache/HAL/clocks et `MX_*_Init()`, demarre TIM5/TIM12 et FatFs, puis appelle `brick6_app_init()`. Celui-ci execute shared SDRAM, init audio non demarree, init systeme/canonique/storage/UI, publication du snapshot initial, demarrage DMA audio, puis start systeme LED/MIDI.
- `brick6_system_domain_process()` porte maintenant l'agregation superloop: services systeme periodiques, services SD/storage, publication snapshot/commande locale vers audio, drain MIDI audio->systeme, service USB host Board, poll MIDI host borne, cadence UI/OLED/flush. Les `main.c` premium/low-cost n'orchestrent plus directement ces services.
- Domaine shared actuel: SDRAM/FMC runtime via `SDRAM_Init()` apres l'init FMC generee; clocks, alimentation, MPU/cache et `MX_*` restent dans `main.c` tant que le firmware H743 mono-coeur garde un seul point d'entree CubeMX.
- Domaine audio actuel: codec Board, mixer, policy FX, buffers float, engines Drum/Sampler/Looper/Wave, runtime audio, snapshots/commandes/telemetrie cote audio, `audio_init()`, callback DSP et `audio_start()`. Le DMA audio ne demarre qu'apres publication forcee du snapshot initial par le domaine systeme.
- Domaine systeme actuel: USB device/role Board, `sd_access_gate`, waveform/cache/catalogue, reservoirs RAW Looper, writer, pools Sampler/Multi/RAM, engine tasklet, params/defaults, sequenceur, UI, pattern/live, patch/kit/project, undo, control events, Hall/calibration, LED et MIDI.
- Ownership futur peripheriques: shared init unique = clocks/alimentation/MPU-cache/FMC-SDRAM; M7 = SAI/I2S audio, DMA audio, codec, pipeline audio; M4 = UI/OLED/LED/Hall/ADC controls, MIDI/USB, SDMMC/FatFs, loaders/cache/writer/persistence. Les timers TIM5/TIM12 restent systeme/M4-cibles avec contrat temporel a projeter vers M7 si necessaire.
- Dependances croisees conservees volontairement: les `main.c` generes initialisent encore tous les peripheriques dans le firmware unique; FatFs est initialise dans `main()` avant les services storage; les handles et IRQ CubeMX ne sont pas reclasses tant qu'aucun projet CM7/CM4, HSEM ou memoire inter-coeur n'existe.
