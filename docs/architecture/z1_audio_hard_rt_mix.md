...
- `Src/Sampler/multi_sample_loader.c` + `Inc/Sampler/multi_sample_loader.h` : LOAD cooperatif du futur `Sampler/Multi`, hors IRQ, qui mappe `.brickmulti` vers `multi_sample_pool` puis prepare page0..page7 protegees pour chaque sample reel via le `sample_page_cache`/`sample_stream_manager` uniques.
- `Src/Sampler/sample_stream_manager.c` + `Inc/Sampler/sample_stream_manager.h` : seam STREAM Sampler; phase courante = proprietaire de la policy service STREAM pool, d'un pool statique de readers FatFs persistants par cle audio STREAM active, et d'une file pending ordonnee par distance de page. Son service est cooperatif: il limite pages/operations FatFs/ticks par appel, sans boost urgent multi-page ni seconde tranche superloop cachee.
- `Src/Sampler/sample_page_cache.c` + `Inc/Sampler/sample_page_cache.h` : seam local du cache pagine Sampler; en phase actuelle, `READY_FULL` peut etre charge par pages float stereo contigues en SDRAM sans modifier le chemin audio stream, et le stockage/acquire/release des pages reste ici. Le lookup passe par un index statique borne keyed par `sample_audio_key_t {domain, object_id}` + `page_index`; les scans libres de zone restent bornes par curseur.
...
- `sample_page_cache` porte aussi la metadata `STREAM_SAFE` par `sample_audio_key_t`: le scan contiguite CLMT/FatFs est isole dans `sample_stream_fatfs_map`, declenche a l'enregistrement d'une source STREAM WAV, et renseigne si elle est candidate `STREAM_SAFE_CONTIGUOUS`.
- Le backend `STREAM_SAFE_CONTIGUOUS` est tente par `sample_stream_manager_service()` avant le chemin FatFs pour les sources safe/compatibles. Il utilise `sd_block_device_read_sectors()` avec contrat gate caller-owned: le `sample_stream_manager` doit deja posseder `sd_access_gate` et le wrapper refuse les appels sans gate ou depuis ISR. Le scratch DMA/cache-safe dedie au streamer vit dans `sample_stream_sector_scratch` en `STORAGE_SCRATCH_SDRAM` (16384 octets, aligne 32, multiple secteur). Tout refus/echec du backend secteur retombe sur le chemin FatFs existant; le page-cache reste l'unique source READY lue par l'audio.
- Le backend secteur V1 supporte uniquement les pages WAV PCM/extensible 48 kHz mono/stereo 16/24/32-bit qui tiennent dans le scratch courant; les cas PCM32 non alignes qui depassent 16384 octets ne sont pas chunkes et basculent volontairement vers FatFs.
- Les certificats `STREAM_SAFE` sont runtime-only: aucun index safe n'est persiste entre boots/projets. Tout register STREAM reconstruit le certificat depuis le WAV reel; tout clear/unregister de cle page-cache l'efface. Le clear d'un instrument Multi libere aussi les readers/pending et pages `domain=MULTI`, donc aucun certificat ancien ne survit a un unload/reload normal.
- Les bancs GDB streamer/cache et compteurs RAM temporaires sont retires. La validation produit du streamer se fait en Release, empiriquement a l'oreille, avec le CPU load existant conserve.
- Le scheduler pending du `sample_stream_manager` sert des demandes explicites `PRESOCLE` ou `ACTIVE`; il n'expose plus de queue background contractuelle ni de lookahead lointain hors fenetre active.
...
- Capacite physique actuelle: `SAMPLE_PAGE_MAX_COUNT` reste le plafond de pages RAM READY/QUEUED/LOADING simultanees tous domaines confondus. Avec la config 16 MiB / pages stereo float de 2048 frames, le plafond theorique est 1024 pages. Multi demande page0..page7 au LOAD comme pre-socle protege.
- Le budget global reste fixe a 16 MiB; le passage a 2048 frames/page reduit le pool physique de 2048 pages de 8 KiB a 1024 pages de 16 KiB, sans augmenter la RAM globale.
...
- Le seuil `READY_FULL` Classic est borne par le cout statique d'un long-stream Classic: `SAMPLE_CACHE_STREAM_STATIC_PAGES = 3`, soit `3 * SAMPLE_PAGE_BYTES = 49152 B = 48 KiB`, donc `SAMPLE_CACHE_FULL_MAX_FRAMES = 6144` frames stereo float decodees. Tout sample Classic au-dessus passe en `READY_PARTIAL`/STREAM, meme si l'ancien seuil 64 pages l'aurait charge en full. Le warm set STREAM initial contient page0 pour le depart, page1 pour le lookahead forward initial, et la page finale pour les entrees reverse/pingpong pres de la fin.
...
- Une page Looper manquante n'arrete pas le curseur audio: le playhead avance sur le silence local, et le refill hors IRQ demande une fenetre de pages en avance, avec wrap modulo vers le debut avant le retour a zero.
...
- La capacite produit Multi testee est `BRICK6_MULTI_MAX_PREPARED_SAMPLES`, soit 64 samples avec `PRESOCLE=512` et `BRICK6_MULTI_PRESOCLE_PAGES=8`. Un instrument `READY` garantit page0..page7 en pre-socle protege pour tous les samples reels longs.
...
- La fenetre active Multi est entretenue hors IRQ via `brick6_sampler_runtime_queue_stream_pages()` et `sample_stream_manager_queue_active_pages()`. Chaque voix Multi active expose sa position avec une cle `domain=MULTI`, `object_id=multi_sample_id`; seules les pages de la fenetre active courante sont demandees.
...
- L'etat produit minimal par track Multi est porte par `brick6_sampler_runtime`: `multi_instrument_id` et `gain`. `multi_sample_pool` reste l'autorite globale des instruments/samples/zones, mais ne porte pas l'assignation track.
...
- `Sampler/Multi` consomme maintenant le note-off clavier via `brick6_sampler_runtime_note_off_multi_track_note(track,note)`: les voix Multi actives du meme couple track/note passent en release pending et continuent a fournir du signal RAM/page-cache jusqu'a extinction du VCA mixer existant, puis sont stoppees/reset avec diagnostic `REL_DONE`. Aucun FatFs, malloc ni UART n'est ajoute en IRQ; les logs streamer/cache continus sont retires du contrat produit.
- L'entretien des pages Multi reste hors IRQ et borne par la fenetre active; l'etat monotone par voix sert uniquement a eviter les requetes redondantes et est reset au trigger/steal/stop/release done.
- La page d'ancrage Multi `page0` n'est plus candidate a l'eviction du `sample_page_cache`; dans le contrat actuel, page0..page7 sont garanties en `PRESOCLE` et les refus restent des etats internes sans log UART streamer/cache continu.

## Addendum 2026-05-15 - noyau STREAM actif commun

- Le noyau STREAM actif classe les pages par distance dans la fenetre active; cette distance pilote l'ordre de service des pages `ACTIVE`, sans classes urgent/background ni boost.
...
- `sample_cache_queue_active_stream_pages()` conserve son rôle legacy Classic/Clip/OneShot, mais délègue maintenant le calcul page courante/fenetre au noyau commun sans dépendance nouvelle à `sample_cache_voice_t`.
- `Sampler/Multi` expose ses voix actives au même noyau via `domain=MULTI`, `object_id=multi_sample_id`, `current_frame=reader.position`, `end_frame=region_end`, direction forward et fenetre active `BRICK6_STREAM_ACTIVE_WINDOW_PAGES`; la policy locale page2/urgent separee est retiree.
- L'anti-spam monotone par voix Multi vit dans `sample_stream_active_state_t` et reste hors IRQ. READY Multi garantit page0..page7 en `PRESOCLE`.
...
## Addendum 2026-05-18 - Diagnostic reloop sequenceur cote audio

- `audio.c` marque les blocs audio qui contiennent un `SEQ_RUNTIME_AUDIO_EVENT_BOUNDARY_EDGE` et pousse des snapshots `SEQEVT` collect/apply vers le ring debug sequenceur.
- Le chemin IRQ audio ne formate pas et ne transmet pas en UART: il copie uniquement des entiers bornes dans le ring statique de Z4.
- Le flush UART reste hors IRQ via `seq_runtime_reloop_debug_flush()` appele depuis la supervision sequenceur en superloop.
...
## Addendum 2026-05-18 - scission PRESOCLE / ACTIVE du page-cache Sampler

- Le pool `sample_page_cache` de 1024 pages / 16 MiB est scinde physiquement en deux zones deterministes: pages `0..511` = `PRESOCLE`, pages `512..1023` = `ACTIVE`.
- `BRICK6_PAGECACHE_PRESOCLE_PAGES=512`, `BRICK6_PAGECACHE_ACTIVE_PAGES=512`, `BRICK6_MULTI_PRESOCLE_PAGES=8`, `BRICK6_STREAM_ACTIVE_WINDOW_PAGES=20`, `BRICK6_MULTI_MAX_PREPARED_SAMPLES=64`.
- Une page `PRESOCLE` est allouee uniquement pour les pages garanties de demarrage d'un sample prepare. Pour `Sampler/Multi`, le LOAD demande page0..page7 via le streamer commun et refuse l'instrument si le quota presocle est depasse.
- Une page `ACTIVE` est allouee pour la fenetre de fonctionnement d'une voix active. La fenetre initiale reste `current_page..current_page+19`; la taille de zone `ACTIVE` est une capacite reservee distincte de cette fenetre par voix. Avec 16 voix, le besoin cible est 320 pages, donc 192 pages restent en marge dans la zone `ACTIVE`.
