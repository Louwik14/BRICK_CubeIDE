...
`VOICES` et `SPREAD` sont des paramètres CFG track-aware, éditables pour `Sampler/Multi` dans les bornes `1..8` et `0..1`. Leur autorité est le runtime Multi; les valeurs Synth restent dans le pool Synth séparé de 16 voix. Pattern, Snapshot, Patch et Kit restaurent ces valeurs par le setter canonique. Ces deux paramètres CFG sont hors Undo/Redo, désormais limité aux mutations structurelles de steps.
...
## 14.c Addendum 2026-08-02 - alignement Mutable sur Deluge stable

La référence Deluge utilisée pour l'audit est `Inspiration/DelugeFirmware-main`, commit
`8907d5b37492cb2749b79a0f3ae1f767bc1336f4` (`stable`). Le code Deluge fonctionne à
44,1 kHz; BRICK fonctionne à 48 kHz et conserve les durées physiques en adaptant les
longueurs et offsets du réseau par `48000 / 44100`.

| Élément | Deluge | BRICK avant correction | Impact possible | Décision |
|---|---|---|---|---|
...
- Le modele de voix Multi est borne par `clamp(VOICES, 1, 8)` par track. Une note depassant la limite de la track vole la plus ancienne voix Multi de cette track.
- Le stockage hot des voix Multi est un pool global DTCM de `SAMPLER_MULTI_MAX_GLOBAL_VOICES = 8` entrees, chaque entree portant son `owner_track_id`; le rendu parcourt le pool global et route chaque voix vers sa track proprietaire.
- La limite globale volable du Sampler/Multi est `SAMPLER_MULTI_MAX_GLOBAL_VOICES = 8`: si elle est atteinte, Multi vole d'abord la plus ancienne voix Multi globale, puis la plus ancienne voix RAM volable. Stream et Looper restent proteges.
- L'etat produit minimal par track Multi est porte par `brick6_sampler_runtime`: `multi_instrument_id`, `gain`, `loop_enabled`, `voice_count` et `spread`. `multi_sample_pool` reste l'autorite globale des instruments/samples/zones et porte seulement les metadonnees sample/zone, dont `has_loop/loop_begin/loop_end`.
...
- `Sampler/Multi` consomme maintenant le note-off clavier via `brick6_sampler_runtime_note_off_multi_track_note_all(track,note)`: les voix Multi actives du meme couple track/note passent en release pending et continuent a fournir du signal RAM/page-cache jusqu'a extinction de leur VCA DSP par voix, puis sont stoppees/reset avec diagnostic `REL_DONE`. Aucun FatFs, malloc ni UART n'est ajoute en IRQ; les logs UART Multi sont desactives par defaut.
...
## Addendum 2026-08-02 - mesure finale Multi polyphonique

- `sizeof(multi_voice_dsp_slot_t) = 608` octets; la taille est identique en mono et stereo car le filtre est une union. Le pool statique contient 8 slots, soit `0x1300 = 4 864` octets, place en `.ram_d1_audio` et aligne a 32 octets.
- `g_sampler_multi_voice` mesure `0x1f00 = 7 936` octets, soit 8 entrees de 992 octets en `.dtcm_audio`. `g_poly_filters_hot` reste le pool synth de 16 entrees (`0x3c00 = 15 360` octets).
- Linker final Release: DTCMRAM 102 240 B, RAM_D1 441 536 B, RAM_D2 154 272 B, RAM_D3 39 456 B, SDRAM 28 902 496 B. Linker final Premium: DTCMRAM 103 264 B, RAM_D1 490 880 B, RAM_D2 168 992 B, RAM_D3 39 456 B, SDRAM 29 871 680 B.
- La comparaison des sections map avant/apres confirme le delta Multi RAM_D1 de `+0x1300` (+4 864 octets); les variations DTCM/D2/SDRAM restantes incluent des modifications preexistantes hors chantier. Aucun stockage Multi nouveau n'est place en D2 ou SDRAM.
- Les recherches finales ne trouvent ni second parametre Multi, ni budget synth modifie, ni borne Multi superieure a 8, ni double filtre/VCA. `mixer_process_external_poly_voice()` reste reserve aux moteurs synth poly. Reverse/ping-pong Multi restent refuses deterministiquement et la mesure CPU IRQ reste materielle.
