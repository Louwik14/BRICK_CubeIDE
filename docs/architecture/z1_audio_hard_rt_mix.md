...
- Lecture bloc Sampler stabilisee pour le playback forward simple: `sample_cache_begin_read_block()` expose un segment stereo contigu en RAM. En `READY_FULL` et `READY_PARTIAL`, ce segment provient maintenant du seam `sample_page_cache` via un bloc RAM-only par page; `sample_cache_commit_read_block()` avance explicitement la voix, relache la page courante et arme le lookahead suivant pour le stream pagine. `brick6_sampler_runtime` consomme ce chemin bloc-by-bloc au lieu d'un appel cache par frame.
- Optimisation IRQ actuelle Sampler: le fast path bloc separe maintenant explicitement mono/stereo et saute la construction de fade par-sample quand `fade_in/out == 0`; le slow path `sample_voice_reader` ne recoit un buffer de fade que si un fade est reellement actif.
