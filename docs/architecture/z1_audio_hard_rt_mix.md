...
TRACK 1..8 lit le buffer moteur/externe immutable capture au debut du bloc, avant filtre, VCA, niveau, pan, inserts et sends. REC lit ce meme tap sur la lane de la premiere entite active, par ordre d'identifiant, dont le Streamer utilise `SOURCE=REC`; en l'absence d'une telle lane, la cible vaut silence. LINE et USB lisent directement les lanes physiques decodees du bloc. MASTER est construit avec les autres tracks, leurs sends/returns et la preview, sans la contribution main ni les sends de la track porteuse; XFADE remplace ensuite le bus juste avant la dynamique master. Une seule route MASTER est admise par CONTROL. Les taps TRACK/REC etant immutables pour tout le bloc, les references reciproques ou indirectes ne reinjectent jamais une sortie XFADE dans une cible du meme bloc.
