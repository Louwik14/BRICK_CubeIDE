...
- CONTROL possede UI, MIDI, sequence, parametres, persistance, SD et loaders. AUDIO possede IRQ, admission, voix, moteurs, mixer et page-cache.
...
- Une liberation visible par AUDIO suit `STOP -> invalidation/ACK AUDIO -> FREE CONTROL`.
- Pattern, Project et Patch utilisent exclusivement le codec CONTROL explicite version 3.

## Flux principaux

```text
configuration CONTROL -> validation globale -> projection runtime -> AUDIO
capture TIM5 -> conversion audio -> file datee -> segmentation -> rendu
besoin stream AUDIO -> commande tokenisee -> I/O Storage -> validation AUDIO -> READY
...
- [recorder_sd.md](recorder_sd.md): Recorder, capture Looper, reservation fichier et relecture de prise.
