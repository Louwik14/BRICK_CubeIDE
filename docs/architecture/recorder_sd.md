...
Le meme bus alimente la conversion PCM24 du Recorder et le peak brut par bloc. AUDIO publie `{generation_io, peak_abs_pcm24}` avant capture et surveille, via un contrat Audio REC dedie, le seuil de l'epoch d'armement. Un franchissement est memorise dans un slot borne puis signale a CONTROL une seule fois; CONTROL latch ARM TRIG et reutilise la quantification NOW/BAR/PATTERN de `sample_capture`. Le writer n'est active qu'apres trigger et echeance. Le vu-metre est diagnostique/UI et ne modifie ni le peak brut, ni l'evenement de trigger, ni la waveform de la prise, ni le WAV.

Une seule capture peut etre preparee ou active. La preparation Storage cree reservation et writer; CONTROL publie REC_BUS puis START/STOP dates. L'IRQ mixer appelle directement l'endpoint Recorder AUDIO, copie dans le ring puis publie le head avec une barriere. STOP volontaire fixe immediatement la decision de conserver: AUDIO ferme la production, Storage draine, ecrit le header, synchronise, ferme, renomme puis publie TAKE_READY. Une navigation ulterieure vers REC EDIT ne peut plus transformer cette decision en abandon.

La sortie reelle de la page Sampling publie au contraire une intention `LEAVE_SAMPLING` dans la FIFO UI vers CONTROL. CONTROL resout alors la session courante: PREPARING/PREPARED ferme et supprime la reservation; RECORDING publie STOP AUDIO puis passe en `CANCELING`. Le writer abandonne le PCM et les descriptors qui ne sont pas engages, attend sans abort une WRITE DMA deja partie, puis ferme et supprime `.REC`; il ne construit ni header utilisateur, ni rename, ni TAKE_READY. Le resultat terminal est CANCELED. DRAINING/FINALIZING issus d'un STOP volontaire sont conserves.
...
Le doorbell Recorder n'est rearme que si un descriptor est pret, si le backlog non assigne atteint 8192 octets, si une fermeture rend le dernier fragment admissible, ou si cancel/finalisation/completion exige une progression. Un reliquat inferieur a 8192 octets pendant CAPTURING ne maintient donc pas STORAGE en boucle.

Preview est refusee avant publication de sa demande lorsqu'une capture Sampling est active, qu'un depart Looper est arme, que son Recorder draine/finalise, ou que les leases physiques primaire/auxiliaire prouvent une lecture Looper. Une prise Looper READY sans lease et sans depart engage reste admissible. L'entree effective en Sampling/Looper ferme d'abord toute Preview existante; aucune Preview n'est mise en attente ou rejouee.

Les waveforms fichier ne sont exclues que si leur chemin est la source effectivement mutee. Une source stable peut progresser avec Recorder ou Preview par acquisitions SD serialisees. Les scans d'identite head/tail et les lectures de construction/editor sont repris par tranches d'au plus 4096 octets, avec offsets, hash et accumulateurs conserves entre les passes.
