...
| `mute` | voir gestures mute; entree quick exige `shift_down=1`, `track_select_armed=0`, `BTN_TRANSPOSE_UP` press | lit/ecrit `mute_active/submode/prev_mode`, `hall_mode`, `hall_note_suppressed` | Oui si match (`return 1`) | Tous (stages 3..11) | appels runtime mute (`track_runtime_refresh_track`, `mixer_set_track_mute`, `param_set`) |
