...
Le timestamp PLAY est resolu une seule fois dans le scheduler CONTROL a partir de la boundary nominale de lane. Les occurrences impaires de la lane recoivent le SWING (`0%` droit, `100%` = retard d'un demi-step de lane); cette phase alternee ne depend pas du rebouclage du pattern. Le MICROTIMING est ensuite converti sur la cadence de base et reduit symetriquement par QUANT (`0%` conserve, `100%` annule). ROLL derive tous ses retriggers de cette origine finale; Note FX recoit donc un timestamp deja definitif. Les sources du step suivant sont ouvertes a la boundary precedente pour couvrir le MICROTIMING negatif sans logique temporelle AUDIO. Lorsqu'une source vient seulement d'etre ouverte et que sa premiere occurrence precede l'horizon encore modifiable (lead negatif ou boundary rattrapee apres une avance du curseur AUDIO), cette occurrence est clampee a cet horizon; une source deja publiee n'est jamais rejouee.
...
La preuve de capacite produit fixe 64 voix emettrices dans les deux topologies, 128 NOTE_ON au plus par horizon (deux sources adjacentes) et 256 actions avec les NOTE_OFF de steal. Le staging interne vaut donc exactement 256. Le preflight de fenetre valide horizon, sources, occurrences, staging et contrat FIFO avant toute avance musicale; un depassement interne residuel est enregistre par le fatal central avec code, entite, contexte, demande et capacite.
...
LENGTH est une deadline CONTROL associee a l'output actif. Un steal du ledger SEQ materialise le NOTE_OFF causal au sample du nouveau NOTE_ON (ou avance le NOTE_OFF imminent existant); le tri OFF avant ON ferme donc aussi toutes les sorties derivees par Note FX avant le retrigger. La mort logique centrale d'un output (STOP, steal, fermeture ou panic) notifie SEQ et Note FX dans la meme transition: l'occurrence active et tout STOP imminent correspondant sont invalides. Une extension ne fait que repousser une deadline encore vivante. ROLL reste reference a l'origine du PLAY et ne materialise que ses retriggers imminents. NOTE, VELOCITY, MICROTIMING et ROLL live n'affectent que les occurrences non publiees.
