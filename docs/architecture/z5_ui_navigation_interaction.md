...
- Disponibilit̩ d'ensembles template:
  - `ui_navigation_is_page_available` combine:
    - contrat structurel Z2 (`track_runtime_is_ui_ensemble_available`),
    - r̩solution template locale (`ui_template_family_resolve_active_track`).
  - Z5 ne red̩cide plus seule la pr̩sence des ensembles principaux (`COLORS/TONE/MOD/MIX/PLAY/VCA`).
...
- Z2: `track_runtime_is_ui_ensemble_available` pour le gating de disponibilit̩ d'ensembles.
