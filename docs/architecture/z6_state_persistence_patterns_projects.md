...
2. `multi_record_writer_service(...)`
   - budget courant 16384 octets par appel apres `sample_cache_service(65536U)`.
...
- Le service SD reste ordonne par `brick6_app_process`: le writer record est servi apres une tranche prioritaire `sample_cache_service(65536U)` pour terminer un drain/finalize deja actif sans preempter les pages STREAM audio pending. Quand un SAVE RAW -> WAV est actif, l'export devient prioritaire avec budget `516096U` et suspend refill Looper, pattern load et preview SD. Hors export actif, l'ordre normal redonne une tranche courte a sample cache apres le service runtime Sampler, puis refill Looper, export opportuniste si aucun refill Looper n'est pending, et pattern load. `sample_cache_service` ne service que la plage hot `0..SAMPLE_CACHE_HOT_SAMPLE_CAPACITY-1`; le Looper service la plage transient `SAMPLE_PAGE_CACHE_LOOPER_ID_BASE..SAMPLE_PAGE_CACHE_LOOPER_ID_BASE+SEQ_TRACK_COUNT-1`.
