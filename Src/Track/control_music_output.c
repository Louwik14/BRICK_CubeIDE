...
#define CONTROL_MUSIC_WINDOW_BUCKET_COUNT \
    CONTROL_MUSIC_WINDOW_MAX_FRAMES
#define CONTROL_MUSIC_WINDOW_NONE UINT16_MAX

typedef struct
{
    control_music_transition_t actions[CONTROL_MUSIC_INTERNAL_MAX_HORIZON_BURST];
    ...
    uint32_t order[CONTROL_MUSIC_INTERNAL_MAX_HORIZON_BURST];
    uint16_t count;
} control_music_window_internal_t;

typedef struct
{
    control_music_transition_t actions[CONTROL_MUSIC_EXTERNAL_STAGING_CAPACITY];
    uint16_t next[CONTROL_MUSIC_EXTERNAL_STAGING_CAPACITY];
    uint16_t head[CONTROL_MUSIC_WINDOW_BUCKET_COUNT];
    uint16_t tail[CONTROL_MUSIC_WINDOW_BUCKET_COUNT];
    uint32_t order[CONTROL_MUSIC_EXTERNAL_STAGING_CAPACITY];
    uint16_t count;
...
static uint32_t g_control_music_window_order;
...
static void control_music_output_reset_window_buckets(void)
{
    ...
    g_control_music_window_order = 0U;
}
...
static uint8_t control_music_output_stage(const control_music_transition_t *action)
{
    if ((action == NULL)
            || (action->entity_id >= BRICK_ENTITY_CAPACITY)
            || (control_music_action_kind(action)
                > (uint8_t)CONTROL_MUSIC_ACTION_RETRIGGER)
            || (action->output_handle == 0U) || (action->note >= 128U)
    ...
    const uint16_t offset = (uint16_t)(
        action->due_sample - g_control_music_window_first);
    const uint16_t bucket = offset;
    const uint8_t external = control_music_action_is_external(action);
    control_music_transition_t *actions;
    uint16_t *next;
    uint16_t *head;
    uint16_t *tail;
    uint16_t *count;
    ...
    if (external != 0U)
    {
        ...
        order = g_control_music_window_external.order;
    ...
    else
    {
        ...
        order = g_control_music_window_internal.order;
    ...
    order[index] = g_control_music_window_order++;
...
static uint8_t control_music_output_publish_batch(
    const control_music_transition_t *actions, uint16_t count);
...
static uint8_t control_music_output_publish(const control_music_transition_t *action)
{
    ...
    const control_music_transition_t *actions, uint16_t count)
{
    if (g_control_music_window_active == 0U)
    {
        ...
        if (control_music_output_begin_window(first_sample, 1U) == 0U)
            return 0U;
        control_music_transition_t normalized[CONTROL_MUSIC_OUTPUTS_PER_ENTITY + 1U];
        ...
        for (uint8_t observer = 0U;
            ...
            if (g_control_music_output_death_observer[observer] != NULL)
                g_control_music_output_death_observer[observer](
                    ...
                    g_control_music_output_staged_death[death].semantic_event_id);
                                           ...
                                           uint8_t index)
{
    ...
    for (uint8_t i = 0U;
        ...
        if (g_control_music_output_death_observer[i] != NULL)
            g_control_music_output_death_observer[i](entity_id,
                semantic_event_id);
}
...
static int8_t control_music_output_find_semantic(brick_entity_id_t entity_id,
                                                 uint32_t semantic_event_id,
                                                 uint32_t causal_source_id,
                                                 uint32_t generation)
{
    for (uint8_t i = 0U; i < CONTROL_MUSIC_OUTPUTS_PER_ENTITY; ++i)
        if ((control_music_output_ledger()[entity_id][i].alive != 0U)
                && (control_music_output_ledger()[entity_id][i].semantic_event_id
                    == semantic_event_id)
                && (control_music_output_ledger()[entity_id][i].causal_source_id
                    == causal_source_id)
                && (control_music_output_ledger()[entity_id][i].generation
                    == generation))
            return (int8_t)i;
    return -1;
}
                                           ...
                                           uint8_t limit)
{
    ...
    control_music_transition_t stops[CONTROL_MUSIC_OUTPUTS_PER_ENTITY];
...
static uint8_t control_music_output_handle_is_live(uint32_t handle)
{
    for (brick_entity_id_t entity = 0U; entity < BRICK_ENTITY_CAPACITY;
         ++entity)
        for (uint8_t i = 0U; i < CONTROL_MUSIC_OUTPUTS_PER_ENTITY; ++i)
            if ((control_music_output_ledger()[entity][i].alive != 0U)
                    && (control_music_output_ledger()[entity][i].output_handle
                        == handle))
                return 1U;
    return 0U;
}
                                    ...
                                    uint32_t generation)
{
    ...
    {
        if (existing < 0) return 1U;
        const control_music_output_t *const output =
            &control_music_output_ledger()[entity_id][(uint8_t)existing];
        const control_music_transition_t stop = {
    ...
    if (existing >= 0)
    {
        control_music_output_t *const output =
            &control_music_output_ledger()[entity_id][(uint8_t)existing];
        control_music_transition_t retrigger = {
            .due_sample = intent->due_sample,
            .output_handle = output->output_handle,
            .entity_id = entity_id,
            .note = intent->note,
        ...
        output->note = intent->note;
        output->velocity = intent->velocity;
    ...
    control_music_transition_t admitted_start = {
        ...
        .entity_id = entity_id,
        .kind = (uint8_t)(CONTROL_MUSIC_ACTION_START
            | (intent->kind & (CONTROL_MUSIC_ACTION_EXTERNAL_FLAG
                | CONTROL_MUSIC_ACTION_CHANNEL_MASK))),
        .note = intent->note,
        .velocity = intent->velocity
    ...
    control_music_transition_t batch[CONTROL_MUSIC_OUTPUTS_PER_ENTITY + 1U];
    ...
    control_music_output_ledger()[entity_id][(uint8_t)target] =
        (control_music_output_t){
            .semantic_event_id = intent->semantic_event_id,
            .output_handle = output_handle,
            ...
            .midi_channel = control_music_intent_channel(intent),
...
{
    if ((intent == NULL) || (intent->entity_id >= BRICK_ENTITY_CAPACITY)
            || (intent->semantic_event_id == 0U)) return 0U;
    control_music_output_t *const outputs =
        control_music_output_ledger()[intent->entity_id];
    for (uint8_t i = 0U; i < CONTROL_MUSIC_OUTPUTS_PER_ENTITY; ++i)
        if ((outputs[i].alive != 0U) && (outputs[i].note == intent->note)
                && (outputs[i].midi_channel
                    == control_music_intent_channel(intent)))
        {
            const control_music_transition_t retrigger = {
                .due_sample = intent->due_sample,
                .output_handle = outputs[i].output_handle,
                .entity_id = intent->entity_id,
                .kind = (uint8_t)(CONTROL_MUSIC_ACTION_RETRIGGER
                    | (intent->kind & (CONTROL_MUSIC_ACTION_EXTERNAL_FLAG
                        | CONTROL_MUSIC_ACTION_CHANNEL_MASK))),
                .note = intent->note,
                .velocity = intent->velocity
            };
            if (control_music_output_publish(&retrigger) == 0U) return 0U;
            control_music_output_send_midi_off(&outputs[i]);
            outputs[i].semantic_event_id = intent->semantic_event_id;
            outputs[i].causal_source_id = causal_source_id;
            outputs[i].generation = generation;
            outputs[i].velocity = intent->velocity;
            outputs[i].age = (g_control_music_window_active != 0U)
                ? ++g_control_music_output_age_staged
                : ++g_control_music_output_age;
            outputs[i].midi_dest_mask = midi_note_on_admit(
                MIDI_DEST_BOTH, outputs[i].midi_channel, outputs[i].note,
                outputs[i].velocity);
            return 1U;
        }
    return control_music_output_submit(intent, causal_source_id, generation);
}
