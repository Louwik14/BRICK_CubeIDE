...
uint8_t mixer_routing_control_set_route(uint32_t track_id, mixer_route_t route)
{
    if (track_id >= MIXER_MAX_TRACKS) return 0U;
                                              ...
                                              int8_t slot)
{
    if ((track_id >= MIXER_MAX_TRACKS)
            || (insert_idx >= MIXER_INSERTS_PER_TRACK)) return 0U;
    mixer_routing_publish_begin();
                                             ...
                                             mixer_routing_snapshot_t *out)
{
    ...
    for (uint8_t attempt = 0U; attempt < 2U; ++attempt)
    {
        ...
        for (uint32_t track = 0U; track < MIXER_MAX_TRACKS; ++track)
        {
            out->route_master[track] = g_mixer_routing_mailbox.route_master[track];
            for (uint32_t insert = 0U; insert < MIXER_INSERTS_PER_TRACK; ++insert)
