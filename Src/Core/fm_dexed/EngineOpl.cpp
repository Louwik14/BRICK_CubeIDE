...
void EngineOpl::compute(int32_t *output, const int32_t *input, int32_t phase0, int32_t freq,
...
void EngineOpl::compute_pure(int32_t *output, int32_t phase0, int32_t freq,
                             int32_t gain1, int32_t gain2, bool add, int frames) {
    int32_t dgain = (gain2 - gain1 + (frames >> 1)) / frames;
    int32_t gain = gain1;
    int32_t phase = phase0;
            ...
            if (inbus == 0 || !has_contents[inbus]) {
                ...
                if ((flags & 0xc0) == 0xc0 && feedback_shift < 16) {
                    ...
                    compute_fb(outptr, param.phase, param.freq,
                                           gain1, gain2,
                                           fb_buf, feedback_shift, add, frames);
                } else {
                    // cout << op << " pure " << inbus << outbus << add << endl;
                    compute_pure(outptr, param.phase, param.freq,
                                             gain1, gain2, add, frames);
                }
            } else {
                // cout << op << " normal " << inbus << outbus << " " << param.freq << add << endl;
                compute(outptr, buf_[inbus - 1].get(),
                                    param.phase, param.freq, gain1, gain2, add, frames);
            }
