...
void EngineMkI::compute(int32_t *output, const int32_t *input,
                        ...
                        int32_t gain1, int32_t gain2, bool add, int frames) {
    int32_t dgain = (gain2 - gain1 + (frames >> 1)) / frames;
    int32_t gain = gain1;
...
void EngineMkI::compute_fb(int32_t *output, int32_t phase0, int32_t freq,
                           ...
                           int32_t *fb_buf, int fb_shift, bool add, int frames) {
    int32_t dgain = (gain2 - gain1 + (frames >> 1)) / frames;
    int32_t gain = gain1;
    int32_t phase = phase0;
    const int32_t *adder = add ? output : zeros;
...
void EngineMkI::compute_fb2(int32_t *output, FmOpParams *parms, int32_t gain01,
                            int32_t gain02, int32_t *fb_buf, int fb_shift, int frames) {
    int32_t dgain[2];
    int32_t gain[2];
    int32_t phase[2];
    int32_t y0 = fb_buf[0];
    ...
    dgain[0] = (gain02 - gain01 + (frames >> 1)) / frames;
...
void EngineMkI::compute_fb3(int32_t *output, FmOpParams *parms, int32_t gain01,
                            int32_t gain02, int32_t *fb_buf, int fb_shift, int frames) {
    int32_t dgain[3];
    int32_t gain[3];
    int32_t phase[3];
            ...
            if (inbus == 0 || !has_contents[inbus]) {
                ...
                if ((flags & 0xc0) == 0xc0 && fb_on) {
                    switch ( algorithm ) {
                        ...
                        case 3 :
                            compute_fb3(outptr, params, gain1, gain2, fb_buf, min((int)(feedback_shift + 2), 16), frames);
                            params[1].phase += params[1].freq * frames; // hack, we already processed op-5 - op-4
                            params[2].phase += params[2].freq * frames; // yuk yuk
                            op += 2; // ignore the 2 other operators
                            break;
                        // two operator feedback, process exception for ALGO 6
                        case 5 :
                            compute_fb2(outptr, params, gain1, gain2, fb_buf, min((int)(feedback_shift + 2), 16), frames);
                            params[1].phase += params[1].freq * frames;  // yuk, hack, we already processed op-5
                            op++; // ignore next operator;
                            break;
                        case 31 :
                            // one operator feedback, process exception for ALGO 32
                            compute_fb(outptr, param.phase, param.freq, gain1, gain2, fb_buf, min((int)(feedback_shift + 2), 16), add, frames);
                            break;
                        default:
                            // one operator feedback, normal process
                            compute_fb(outptr, param.phase, param.freq, gain1, gain2, fb_buf, feedback_shift, add, frames);
                            break;
                    }
                } else {
                    compute_pure(outptr, param.phase, param.freq, gain1, gain2, add, frames);
                }
            } else {
                compute(outptr, buf_[inbus - 1].get(), param.phase, param.freq, gain1, gain2, add, frames);
            }
