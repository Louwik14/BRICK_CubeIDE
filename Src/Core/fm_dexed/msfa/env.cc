...
int32_t Env::getsample(uint32_t frames) {
#ifdef ACCURATE_ENVELOPE
    if (staticcount_) {
        staticcount_ -= (int)frames;
        if (staticcount_ <= 0) {
            staticcount_ = 0;
            advance(ix_ + 1);
        }
    ...
    if (ix_ < 3 || ((ix_ < 4) && !down_)) {
        if (staticcount_) {
            ...
            const int32_t scaled_inc =
                (int32_t)(((int64_t)inc_ * frames + inc_remainder_) / N);
            inc_remainder_ =
                (int32_t)(((int64_t)inc_ * frames + inc_remainder_) % N);
            level_ += (((17 << 24) - level_) >> 24) * scaled_inc;
