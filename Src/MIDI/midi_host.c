...
{
  /* Drain request is consumed by the cooperative USB service. */
...
void midi_host_transport_poll_bounded(uint32_t max_msgs)
{
  if (midi_host_discard_requested != 0U) {
    if (usb_host_is_ready() != 0U) {
      ...
      while ((discarded < max_msgs)
             && tuh_midi_packet_read(0U, midi_host_rx_packet)) {
        ++discarded;
      }
      if (tuh_midi_read_available(0U) >= MIDI_HOST_PACKET_SIZE) {
        return;
      }
    }
    midi_host_event_tail = midi_host_event_head;
