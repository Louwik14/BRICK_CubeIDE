                                                   ...
                                                   uint32_t frames)
{
    ...
    {
        g_dual.feedback = smooth_changed_parameter(g_dual.feedback,
                                                   g_dual.feedback_target,
                                                   &g_dual.feedback_smooth_remaining);
        g_dual.width = smooth_changed_parameter(g_dual.width,
                                                g_dual.width_target,
                                                &g_dual.width_smooth_remaining);
        g_dual.volume = smooth_changed_parameter(g_dual.volume,
                                                 g_dual.volume_target,
                                                 &g_dual.volume_smooth_remaining);
        const float feedback_scale = (fabsf(feedback_reference) > kNeutralEpsilon)
            ? (g_dual.feedback / feedback_reference) : 0.0f;
        const float feedback_l_now = feedback_l * feedback_scale;
        const float feedback_r_now = feedback_r * feedback_scale;
        ...
        const float pipo_width = (mode == (uint8_t)FX_DELAY_DUAL_MODE_CLASSIC_PINGPONG)
            ? clampf_local(fbw, -1.0f, 1.0f) : width;
        ...
        if(mode == (uint8_t)FX_DELAY_DUAL_MODE_NORMAL)
        {
            ...
            delay_line_write(&g_dual.delay_l, sanitize_feedback_sample(in_l[i] + (fb_main_l * feedback_l_now)), 0U);
            delay_line_write(&g_dual.delay_r, sanitize_feedback_sample(in_r[i] + (fb_main_r * feedback_r_now)), 0U);
        }
                ...
                || (mode == (uint8_t)FX_DELAY_DUAL_MODE_CLASSIC_PINGPONG))
        {
            ...
            delay_line_write(&g_dual.delay_l, sanitize_feedback_sample((mono * lfactor) + (fb_delay_l * feedback_l_now)), 0U);
            delay_line_write(&g_dual.delay_r, sanitize_feedback_sample((mono * rfactor) + (fb_delay_r * feedback_r_now)), 0U);
        }
        else
        {
            const float wet_fb_l = v0;
            ...
            const float wet_filt_l = feedback_filter_process(&g_dual.filter_delay_l, wet_fb_l, hpf_active, lpf_active, hp_a, lp_a);
            const float wet_filt_r = feedback_filter_process(&g_dual.filter_delay_r, wet_fb_r, hpf_active, lpf_active, hp_a, lp_a);
            delay_line_write(&g_dual.delay_l, sanitize_feedback_sample(in_l[i] + (wet_filt_l * feedback_l_now)), 0U);
            delay_line_write(&g_dual.delay_r, sanitize_feedback_sample(in_r[i] + (wet_filt_r * feedback_r_now)), 0U);
        }
                ...
                && (mode != (uint8_t)FX_DELAY_DUAL_MODE_CLASSIC_PINGPONG))
        {
            ...
            if(has_haas != 0U)
            {
                const float max_haas_samples = kMaxHaasMs * sr * (1.0f / 1000.0f);
                const float haas_l = (width < 0.0f) ? (1.0f + (-width * max_haas_samples)) : 1.0f;
                const float haas_r = (width > 0.0f) ? (1.0f + ( width * max_haas_samples)) : 1.0f;
                wet_l = delay_line_read_linear(&g_dual.haas_l, haas_l);
                wet_r = delay_line_read_linear(&g_dual.haas_r, haas_r);
            }
        }
