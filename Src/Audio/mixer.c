...
/* AUDIO-owned track projection.  The unsuffixed values below are targets
 * received through the CONTROL -> AUDIO parameter path; *_current values,
 * ramps and DSP state never leave this module. */
...
/* Filter/VCA AUDIO runtime. Canonical UI/ParamStore values live in
 * track_sound_state; this object contains only the applied projection and
 * current DSP/envelope state. */
typedef struct __attribute__((aligned(32))) {
    union {
...
/**
 * @brief Routing track publie par le domaine CONTROL.
 *
 * Role:
 * - Le setter direct AUDIO a ete retire en PASS 3B.
 *
 * @param track_id Parametre d'entree de l'API.
...
/**
 * @brief Slots insert publies par le domaine CONTROL.
 *
 * Role:
 * - Le setter direct AUDIO a ete retire en PASS 3B.
 *
