...
typedef struct { volatile uint8_t model; volatile float p1,p2,p3; audio_fx_runtime_dsp_t dsp; } audio_fx_runtime_t;
...
{
    switch(r->model){case AUDIO_FX_MODEL_LOFI:fx_audio_lofi_reset(&r->dsp.lofi);break;
    ...
    if(r->model==AUDIO_FX_MODEL_SUB||r->model==AUDIO_FX_MODEL_SUB_LIGHT)fx_audio_sub_reset(&r->dsp.sub);
    else if(r->model==AUDIO_FX_MODEL_RING)fx_audio_ring_reset(&r->dsp.ring);
}
static void prepare(audio_fx_runtime_t*r)
{
    switch(r->model){case AUDIO_FX_MODEL_LOFI:
        fx_audio_lofi_set_engine(&r->dsp.lofi,(fx_audio_lofi_engine_t)audio_fx_lofi_model_index_from_control((uint8_t)(clamp127(r->p3)+0.5f)));
        fx_audio_lofi_prepare(&r->dsp.lofi,r->p1,r->p2);break;
    case AUDIO_FX_MODEL_FOLD:fx_audio_fold_prepare(&r->dsp.fold,r->p1,r->p2,clamp127(r->p3)/127.0f);break;
    case AUDIO_FX_MODEL_DRIVE:fx_audio_drive_prepare(&r->dsp.drive,r->p1,r->p2,clamp127(r->p3));break;
    case AUDIO_FX_MODEL_POINT:fx_audio_point_prepare(&r->dsp.point,r->p1,r->p2,clamp127(r->p3)/127.0f,(float)BOARD_AUDIO_SAMPLE_RATE_HZ);break;
    case AUDIO_FX_MODEL_SUB:case AUDIO_FX_MODEL_SUB_LIGHT:fx_audio_sub_prepare(&r->dsp.sub,r->p1,r->p2,clamp127(r->p3)/127.0f,(float)BOARD_AUDIO_SAMPLE_RATE_HZ);break;
    case AUDIO_FX_MODEL_RING:fx_audio_ring_prepare(&r->dsp.ring,r->p1,r->p2,clamp127(r->p3),(float)BOARD_AUDIO_SAMPLE_RATE_HZ);break;
    default:break;}
}
static float mono_one(audio_fx_runtime_t*r,float x)
{
    switch(r->model){case AUDIO_FX_MODEL_LOFI:return fx_audio_lofi_process_mono_sample(&r->dsp.lofi,x);
    ...
    if(r->model==AUDIO_FX_MODEL_SUB)return fx_audio_sub_process_mono_sample(&r->dsp.sub,x);
    if(r->model==AUDIO_FX_MODEL_SUB_LIGHT)return fx_audio_sub_light_process_mono_sample(&r->dsp.sub,x);
    if(r->model==AUDIO_FX_MODEL_RING)return fx_audio_ring_process_mono_sample(&r->dsp.ring,x);
    return x;
}
...
uint8_t audio_fx_runtime_is_active(brick_entity_id_t e){return e<BRICK_ENTITY_CAPACITY&&g_audio_fx_runtime[e].model!=AUDIO_FX_MODEL_OFF;}
...
uint8_t audio_fx_runtime_apply_param(brick_entity_id_t e,param_id_t id,float value)
{
    ...
    if(id==PARAM_AUDIO_FX_P1){r->p1=clamp01(value);prepare(r);}
    else if(id==PARAM_AUDIO_FX_P2){r->p2=clamp01(value);prepare(r);}
    else if(id==PARAM_AUDIO_FX_P3){r->p3=clamp127(value);if(r->model==AUDIO_FX_MODEL_LOFI||r->model==AUDIO_FX_MODEL_FOLD||r->model==AUDIO_FX_MODEL_DRIVE||r->model==AUDIO_FX_MODEL_POINT||r->model==AUDIO_FX_MODEL_SUB||r->model==AUDIO_FX_MODEL_SUB_LIGHT||r->model==AUDIO_FX_MODEL_RING)prepare(r);}
    else {const uint8_t m=valid_model(value);if(r->model!=m){r->model=m;reset_dsp(r);prepare(r);}}return 1U;
}
void audio_fx_runtime_process_mono(brick_entity_id_t e,float*b,uint32_t n)
{
    ...
    audio_fx_runtime_t*r=&g_audio_fx_runtime[e];
    if(r->model==AUDIO_FX_MODEL_LOFI){fx_audio_lofi_process_mono(&r->dsp.lofi,b,n);return;}
    if(r->model==AUDIO_FX_MODEL_POINT){fx_audio_point_process_mono(&r->dsp.point,b,n);return;}
...
void audio_fx_runtime_process_stereo(brick_entity_id_t e,float*l,float*rgt,uint32_t n)
{
    ...
    audio_fx_runtime_t*r=&g_audio_fx_runtime[e];
    if(r->model==AUDIO_FX_MODEL_LOFI)fx_audio_lofi_process_stereo(&r->dsp.lofi,l,rgt,n);
    else if(r->model==AUDIO_FX_MODEL_FOLD)fx_audio_fold_process_stereo(&r->dsp.fold,l,rgt,n);
    else if(r->model==AUDIO_FX_MODEL_DRIVE)fx_audio_drive_process_stereo(&r->dsp.drive,l,rgt,n);
    else if(r->model==AUDIO_FX_MODEL_POINT)fx_audio_point_process_stereo(&r->dsp.point,l,rgt,n);
    else if(r->model==AUDIO_FX_MODEL_SUB)for(uint32_t i=0;i<n;++i)fx_audio_sub_process_stereo_sample(&r->dsp.sub,&l[i],&rgt[i]);
    else if(r->model==AUDIO_FX_MODEL_SUB_LIGHT)for(uint32_t i=0;i<n;++i)fx_audio_sub_light_process_stereo_sample(&r->dsp.sub,&l[i],&rgt[i]);
    else if(r->model==AUDIO_FX_MODEL_RING)for(uint32_t i=0;i<n;++i)fx_audio_ring_process_stereo_sample(&r->dsp.ring,&l[i],&rgt[i]);
}
void audio_fx_runtime_process_stereo_sample(brick_entity_id_t e,float*l,float*r)
{if(e>=BRICK_ENTITY_CAPACITY||!l||!r||!audio_fx_runtime_is_active(e))return;audio_fx_runtime_t*s=&g_audio_fx_runtime[e];if(s->model==AUDIO_FX_MODEL_LOFI)fx_audio_lofi_process_stereo_sample(&s->dsp.lofi,l,r);else if(s->model==AUDIO_FX_MODEL_FOLD){*l=fx_audio_fold_process_mono_sample(&s->dsp.fold,*l);*r=fx_audio_fold_process_mono_sample(&s->dsp.fold,*r);}else if(s->model==AUDIO_FX_MODEL_DRIVE){*l=fx_audio_drive_process_sample(&s->dsp.drive,*l);*r=fx_audio_drive_process_sample(&s->dsp.drive,*r);}else if(s->model==AUDIO_FX_MODEL_POINT)fx_audio_point_process_stereo_sample(&s->dsp.point,l,r);else if(s->model==AUDIO_FX_MODEL_SUB)fx_audio_sub_process_stereo_sample(&s->dsp.sub,l,r);else if(s->model==AUDIO_FX_MODEL_SUB_LIGHT)fx_audio_sub_light_process_stereo_sample(&s->dsp.sub,l,r);else if(s->model==AUDIO_FX_MODEL_RING)fx_audio_ring_process_stereo_sample(&s->dsp.ring,l,r);}
