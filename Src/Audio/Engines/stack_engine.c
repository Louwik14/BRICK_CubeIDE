...
#define STACK_SQUARE_RENDER_BLOCK_SIZE 24U
    ...
    uint32_t frames)
{
    ...
    while (offset < frames)
    {
        ...
        if ((osc->renderer_id == STACK_RENDERER_DELUGE_SQUARE)
                && (chunk > STACK_SQUARE_RENDER_BLOCK_SIZE))
            chunk = STACK_SQUARE_RENDER_BLOCK_SIZE;
    ...
    uint32_t frames)
{
    ...
    const uint32_t ramp_frames = (frames > STACK_SQUARE_RENDER_BLOCK_SIZE)
        ? STACK_SQUARE_RENDER_BLOCK_SIZE : frames;
