...
void FmOpKernel::compute(int32_t *output, const int32_t *input,
  ...
  if (hasNeon()) {
#ifdef HAVE_NEON
    neon_fm_kernel(input, add ? output : zeros, output, frames,
...
void FmOpKernel::compute_pure(int32_t *output, int32_t phase0, int32_t freq,
  ...
  if (hasNeon()) {
#ifdef HAVE_NEON
    neon_fm_kernel(zeros, add ? output : zeros, output, frames,
      phase0, freq, gain, dgain);
#endif
  } else {
    if (add) {
...
// Experimental sine wave generators are intentionally omitted; the active
// implementation above is the only production kernel.
