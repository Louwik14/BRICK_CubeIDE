...
static const ui_template_family_t g_ui_template_env_family_group = {
    .family_title = "ENV",
    .nav_labels = { "FILTER", "-", "-", "ENV 3" },
    .subpages = {
        { .title = "FILTER", .param_bank = { .params = { PARAM_FILTER_CUTOFF, PARAM_FILTER_RESONANCE, PARAM_COUNT, PARAM_FILTER_MORPH } } },
...
{
    if (ui_get_track_type(ui_get_active_lane()) == TRACK_TYPE_GROUP)
    {
        return &g_ui_template_env_family_group;
    }
                                                             ...
                                                             track_type_t active_type)
{
    ...
    const uint8_t filter_morph_ui = (uint8_t)(ui_param_get_active_track_display_value(
        PARAM_FILTER_MORPH, ui_get_active_lane()) + 0.5f);
...
{
    if ((g_ui_template_env_subset != 0U)
            || (ui_get_track_type(ui_get_active_lane()) == TRACK_TYPE_GROUP))
    {
        return;
    }
