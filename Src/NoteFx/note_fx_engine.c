...
typedef struct {uint32_t source_id,generation;int32_t offset;} note_fx_groove_lifetime_t;
...
/* Harmony branches for one product lane cross Groove at the same sample and
 * retain the same source lifetime.  The remembered live offset is therefore
 * lane/slot state, not branch state.  Keeping a branch dimension multiplied
 * this ledger by four without representing any additional lifetime. */
static SEQ_GROOVE_STATE_D2 note_fx_groove_lifetime_t
    g_groove_lifetime[NOTE_FX_SLOT_COUNT][SEQ_PRODUCT_MAX_EMITTING_VOICES];
_Static_assert(sizeof(g_groove_lifetime)==3072U,
    "Groove live-lifetime ledger budget");
...
static note_fx_echo_state_t*echo_state(const note_event_t*e){
 const uint16_t lane=product_lane(e);
...
static uint8_t source_is_live(uint32_t source){const uint32_t ns=source&~NOTE_EVENT_OCCURRENCE_COUNTER_MASK;return(uint8_t)(ns==NOTE_EVENT_OCCURRENCE_NAMESPACE_KEY||ns==NOTE_EVENT_OCCURRENCE_NAMESPACE_MIDI);}
static uint64_t sample_add_offset(uint64_t sample,int32_t offset){if(offset<0&&(uint64_t)(-(int64_t)offset)>sample)return 0U;return(uint64_t)((int64_t)sample+offset);}
...
static note_event_result_t held_ingest(uint8_t slot,note_fx_slot_runtime_t*r,const note_event_t*e){const uint16_t lane=product_lane(e);const uint8_t branch=note_event_branch(e);const uint8_t family=held_family(r->model);note_fx_family_runtime_t*f=family_state(e->track,slot);if(!family_owns(e->track,slot))return NOTE_EVENT_RESULT_ACCEPTED;if(lane>=SEQ_PRODUCT_MAX_EMITTING_VOICES||branch>=SEQ_PRODUCT_HARMONY_FANOUT_MAX)return NOTE_EVENT_RESULT_REJECTED_DESTINATION;note_fx_held_pitch_t*h=&g_held[family][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch];if(e->kind==NOTE_EVENT_KIND_OFF){if(h->source_token==e->source_id&&h->generation==e->source_generation)held_remove(family,f,lane,branch);return NOTE_EVENT_RESULT_ACCEPTED;}if(e->duration_samples==0U)return NOTE_EVENT_RESULT_DROPPED_POLICY;if(!h->source_token)++f->held_count;const uint32_t lifetime_end=((e->flags&NOTE_EVENT_FLAG_HELD)!=0U)?0U:(uint32_t)(e->sample_abs+e->duration_samples);*h=(note_fx_held_pitch_t){.source_token=e->source_id,.generation=e->source_generation,.group_id=e->group_id,.lifetime_end=lifetime_end,.note=e->note,.velocity=e->velocity,.dependency_mask=e->dependency_mask};g_held_active_mask[family][branch]|=UINT64_C(1)<<lane;(void)r;return NOTE_EVENT_RESULT_ACCEPTED;}
static void held_expire(uint8_t t,uint8_t s,uint64_t sample){const uint8_t family=held_family(g_slot[t][s].model);note_fx_family_runtime_t*f=family_state(t,s);const uint64_t lanes=track_lane_mask(t);for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){uint64_t active=g_held_active_mask[family][branch]&lanes;while(active){const uint16_t lane=(uint16_t)__builtin_ctzll(active);active&=active-1U;note_fx_held_pitch_t*h=&g_held[family][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch];if(held_deadline_reached(h,sample))held_remove(family,f,lane,branch);}}}
static note_fx_held_pitch_t*held_rank(uint8_t t,uint8_t s,note_fx_slot_runtime_t*r,uint8_t rank,uint8_t sorted){note_fx_held_pitch_t*items[SEQ_LOGICAL_CAPACITY_MAX*SEQ_PRODUCT_HARMONY_FANOUT_MAX];uint8_t count=0;const uint8_t family=held_family(r->model);uint64_t active=held_active_lanes(family,track_lane_mask(t));while(active){const uint16_t lane=(uint16_t)__builtin_ctzll(active);active&=active-1U;for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch)if((g_held_active_mask[family][branch]&(UINT64_C(1)<<lane))!=0U)items[count++]=&g_held[family][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch];}if(rank>=count)return NULL;if(sorted)for(uint8_t i=1;i<count;++i){note_fx_held_pitch_t*x=items[i];uint8_t j=i;while(j&&items[j-1U]->note>x->note){items[j]=items[j-1U];--j;}items[j]=x;}return items[rank];}
...
static note_event_result_t direct(uint8_t slot,note_fx_slot_runtime_t*r,const note_event_t*e,note_event_t*out,uint8_t cap,uint8_t*count){const uint8_t stage=(uint8_t)(slot+1U);
 ...
 if(r->model==NOTE_FX_MODEL_GROOVE){note_event_t x=*e;const uint8_t phase=groove_phase(e->track,e->sample_abs);const note_fx_groove_template_t*t=&g_groove[r->p1%NOTE_FX_GROOVE_TYPE_COUNT];int32_t off=(int32_t)(((int64_t)t->timing[phase]*(int64_t)step_samples()*r->p2)/(96LL*100LL));const uint16_t lane=product_lane(e);note_fx_groove_lifetime_t*l=(lane<SEQ_PRODUCT_MAX_EMITTING_VOICES)?&g_groove_lifetime[slot][lane]:NULL;if(source_is_live(e->source_id)&&l){if(e->kind==NOTE_EVENT_KIND_ON){l->source_id=e->source_id;l->generation=e->source_generation;l->offset=off;}else if(l->source_id==e->source_id&&l->generation==e->source_generation){off=l->offset;memset(l,0,sizeof(*l));}}x.sample_abs=sample_add_offset(x.sample_abs,off);if(source_is_live(e->source_id)&&x.sample_abs<e->sample_abs)x.sample_abs=e->sample_abs;if(x.kind==NOTE_EVENT_KIND_ON){int32_t v=x.velocity+((int32_t)t->velocity[phase]*x.velocity*r->p3)/10000;if(v<1)v=1;if(v>127)v=127;x.velocity=(uint8_t)v;}return append(out,cap,count,&x,stage)?NOTE_EVENT_RESULT_ACCEPTED:NOTE_EVENT_RESULT_REJECTED_CAPACITY;}
...
static note_event_result_t echo_process(uint64_t start,uint64_t end,note_fx_emit_fn emit,void*ctx)
{
 ...
 while(work){const uint16_t lane=(uint16_t)__builtin_ctzll(work);work&=work-1U;
  ...
  while(x->active&&x->next_due<end){
   ...
   e.provenance=NOTE_EVENT_SOURCE_FX;e.flags=(uint8_t)((e.flags
       &~NOTE_EVENT_FLAG_HELD)|NOTE_EVENT_FLAG_GENERATED|NOTE_EVENT_FLAG_ECHO);
...
void note_fx_engine_init(void){memset(&g_seq_context,0,sizeof(g_seq_context));memset(g_echo,0,sizeof(g_echo));memset(g_echo_active,0,sizeof(g_echo_active));memset(g_held,0,sizeof(g_held));memset(g_held_active_mask,0,sizeof(g_held_active_mask));memset(g_family,0,sizeof(g_family));memset(g_groove_lifetime,0,sizeof(g_groove_lifetime));for(uint8_t t=0;t<NOTE_FX_TRACK_COUNT;++t)for(uint8_t f=0;f<2U;++f)g_family[t][f].owner_slot=UINT8_MAX;g_seq_context.samples_per_step_q16=UINT32_C(65536);}
...
static void echo_clear_owner(uint8_t t,uint8_t s){for(uint16_t lane=0;lane<SEQ_PRODUCT_MAX_EMITTING_VOICES;++lane){if(product_track(lane)!=t)continue;for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){note_fx_echo_state_t*x=&g_echo[lane][branch];if(x->active&&x->stage==(uint8_t)(s+1U)){x->active=0U;g_echo_active[branch]&=~(UINT64_C(1)<<lane);}}}}
static void groove_clear_owner(uint8_t t,uint8_t s){for(uint16_t lane=0;lane<SEQ_PRODUCT_MAX_EMITTING_VOICES;++lane)if(product_track(lane)==t)memset(&g_groove_lifetime[s][lane],0,sizeof(note_fx_groove_lifetime_t));}
note_event_result_t note_fx_engine_configure(uint8_t t,uint8_t s,uint8_t model,uint8_t p1,uint8_t p2,uint8_t p3){if(t>=NOTE_FX_TRACK_COUNT||s>=NOTE_FX_SLOT_COUNT)return NOTE_EVENT_RESULT_DROPPED_POLICY;note_fx_slot_runtime_t*r=&g_slot[t][s];if(model>=NOTE_FX_MODEL_COUNT)model=NOTE_FX_MODEL_OFF;const uint8_t old_model=r->model;if(old_model!=model){if(old_model==NOTE_FX_MODEL_ECHO)echo_clear_owner(t,s);if(old_model==NOTE_FX_MODEL_GROOVE)groove_clear_owner(t,s);memset(r,0,sizeof(*r));}r->model=model;r->p1=p1;r->p2=p2;r->p3=p3;const uint8_t bit=(uint8_t)(1U<<s);if(model==NOTE_FX_MODEL_OFF)g_context->active_slot_mask[t]&=(uint8_t)~bit;else g_context->active_slot_mask[t]|=bit;if(model==NOTE_FX_MODEL_GROOVE||model==NOTE_FX_MODEL_ECHO||model_is_generator(model))g_context->temporal_slot_mask[t]|=bit;else g_context->temporal_slot_mask[t]&=(uint8_t)~bit;if(model_is_generator(old_model))family_reselect(t,held_family(old_model));if(model_is_generator(model))family_reselect(t,held_family(model));refresh_work(t,s);return NOTE_EVENT_RESULT_ACCEPTED;}
uint8_t note_fx_engine_next_active_slot(uint8_t t,uint8_t stage){if(t>=NOTE_FX_TRACK_COUNT||stage>=NOTE_FX_SLOT_COUNT)return NOTE_FX_SLOT_COUNT;const uint8_t active=(uint8_t)(g_context->active_slot_mask[t]&(uint8_t)(UINT8_MAX<<stage));return active?(uint8_t)__builtin_ctz((unsigned)active):NOTE_FX_SLOT_COUNT;}
uint8_t note_fx_engine_suffix_is_temporal(uint8_t t,uint8_t stage){if(t>=NOTE_FX_TRACK_COUNT||stage>=NOTE_FX_SLOT_COUNT)return 0U;return(uint8_t)((g_context->temporal_slot_mask[t]&(uint8_t)(UINT8_MAX<<stage))!=0U);}
static note_event_result_t transform_core(uint8_t s,const note_event_t*in,uint8_t n,note_event_t*out,uint8_t cap,uint8_t*count,uint8_t validate)
{
 if(!in||!out||!count||!n||s>=NOTE_FX_SLOT_COUNT){
  return NOTE_EVENT_RESULT_DROPPED_POLICY;}
 if(in[0].track>=NOTE_FX_TRACK_COUNT
      ||(validate!=0U?(in[0].stage!=s):(in[0].stage>s))
 ...
 if(r->model==NOTE_FX_MODEL_HARMONIZER)
  return harmonizer_group(s,r,in,n,out,cap,count);
 if(r->model==NOTE_FX_MODEL_CHORD)
  return chord_group(s,r,in,n,out,cap,count);
 *count=0;
 for(uint8_t i=0;i<n;++i){const note_event_t*e=&in[i];
  if(!model_is_arp(r->model)&&r->model!=NOTE_FX_MODEL_EUCLID){
   const note_event_result_t result=direct(s,r,e,out,cap,count);
   if(result!=NOTE_EVENT_RESULT_ACCEPTED)return result;}}
 return NOTE_EVENT_RESULT_ACCEPTED;
}
note_event_result_t note_fx_engine_transform(uint8_t s,const note_event_t*in,uint8_t n,note_event_t*out,uint8_t cap,uint8_t*count){return transform_core(s,in,n,out,cap,count,1U);}
note_event_result_t note_fx_engine_transform_prepared(uint8_t s,const note_event_t*in,uint8_t n,note_event_t*out,uint8_t cap,uint8_t*count){return transform_core(s,in,n,out,cap,count,0U);}
...
static note_event_result_t note_fx_engine_process_due(uint64_t start,uint16_t frames,uint32_t sps,note_fx_emit_fn emit,void*ctx){note_fx_engine_set_samples_per_step_q16(sps);const uint64_t end=start+frames;note_event_result_t echo_result=echo_process(start,end,emit,ctx);if(echo_result!=NOTE_EVENT_RESULT_ACCEPTED)return echo_result;uint64_t processed=0,work;while((work=(g_work_slot_mask&~processed))!=0){const uint32_t idx=(uint32_t)__builtin_ctzll(work);processed|=UINT64_C(1)<<idx;const uint8_t t=(uint8_t)(idx/NOTE_FX_SLOT_COUNT),s=(uint8_t)(idx%NOTE_FX_SLOT_COUNT);note_fx_slot_runtime_t*r=&g_slot[t][s];note_fx_family_runtime_t*f=family_state(t,s);held_expire(t,s,start);refresh_work(t,s);if(!f->held_count||!family_owns(t,s)){continue;}uint64_t period=seq_division_period_samples(r->model==NOTE_FX_MODEL_EUCLID?r->p3:r->p1,sps);if(!period)period=1;const note_fx_phase_policy_t policy=(r->model==NOTE_FX_MODEL_ARP_FREE)?NOTE_FX_PHASE_TRANSPORT_FREE:NOTE_FX_PHASE_PATTERN_SYNC;const uint64_t position=(policy==NOTE_FX_PHASE_TRANSPORT_FREE)?g_context->transport_position_q16:g_context->pattern_position_q16[t];const uint64_t step_sample=step_samples();uint64_t period_q16=(period<<16U)/step_sample;if(!period_q16)period_q16=1U;uint32_t ordinal=(uint32_t)(position/period_q16);const uint64_t remainder=position%period_q16;uint64_t sample=start;if(remainder!=0U){const uint64_t remaining=period_q16-remainder;sample+=((remaining*step_sample)+UINT16_MAX)>>16U;++ordinal;}while(sample<end){held_expire(t,s,sample);if(!f->held_count)break;if(model_is_arp(r->model)){note_fx_held_pitch_t*h=NULL;uint8_t note=0;if(arp_select(r,ordinal,t,s,&h,&note)){const note_event_result_t result=emit_generated(t,s,h,note,sample,period,emit,ctx);if(result!=NOTE_EVENT_RESULT_ACCEPTED){return result;}}}else{const uint8_t length=r->p1?r->p1:NOTE_FX_EUCLID_LENGTH_DEFAULT,pulse=r->p2<=length?r->p2:length;const uint64_t mask=euclid_build_mask(length,pulse);if((mask>>(ordinal%length))&UINT64_C(1)){const uint8_t family=held_family(r->model);uint64_t active=held_active_lanes(family,track_lane_mask(t));while(active){const uint16_t lane=(uint16_t)__builtin_ctzll(active);active&=active-1U;for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){if((g_held_active_mask[family][branch]&(UINT64_C(1)<<lane))==0U)continue;const note_fx_held_pitch_t*h=&g_held[family][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch];const note_event_result_t result=emit_generated(t,s,h,h->note,sample,period,emit,ctx);if(result!=NOTE_EVENT_RESULT_ACCEPTED){return result;}}}}}++ordinal;sample+=period;}refresh_work(t,s);}return NOTE_EVENT_RESULT_ACCEPTED;}
...
void note_fx_engine_forget_dependency(uint8_t t,uint8_t owner){if(t>=NOTE_FX_TRACK_COUNT||owner>=NOTE_FX_SLOT_COUNT)return;const uint8_t bit=(uint8_t)(1U<<owner);const uint64_t lanes=track_lane_mask(t);for(uint8_t family=0;family<2U;++family)for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){uint64_t active=g_held_active_mask[family][branch]&lanes;while(active){const uint16_t lane=(uint16_t)__builtin_ctzll(active);active&=active-1U;note_fx_held_pitch_t*h=&g_held[family][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch];if((h->dependency_mask&bit)!=0U)held_remove(family,&g_family[t][family],lane,branch);}}for(uint8_t s=0;s<NOTE_FX_SLOT_COUNT;++s)refresh_work(t,s);}
void note_fx_engine_forget_causal_sources_from_slot(uint8_t t,uint8_t first,const uint32_t*ids,uint16_t count){if(t>=NOTE_FX_TRACK_COUNT||first>=NOTE_FX_SLOT_COUNT||!ids)return;const uint64_t lanes=track_lane_mask(t);for(uint8_t family=0;family<2U;++family){if(g_family[t][family].owner_slot<first)continue;for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){uint64_t active=g_held_active_mask[family][branch]&lanes;while(active){const uint16_t lane=(uint16_t)__builtin_ctzll(active);active&=active-1U;note_fx_held_pitch_t*h=&g_held[family][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch];for(uint16_t j=0;j<count;++j)if(h->source_token==ids[j]){held_remove(family,&g_family[t][family],lane,branch);break;}}}}for(uint8_t s=0;s<NOTE_FX_SLOT_COUNT;++s)refresh_work(t,s);}
