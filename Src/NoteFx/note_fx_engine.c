...
typedef struct {uint32_t source_token,generation,group_id,lifetime_end;
 uint8_t note,velocity,dependency_mask,destination_id;} note_fx_held_pitch_t;
typedef struct {uint16_t dependency_versions;uint8_t held_count,model,p1,p2,p3;} note_fx_slot_runtime_t;
typedef struct { int8_t timing[8],velocity[8]; } note_fx_groove_template_t;
typedef struct { note_event_t seed;uint64_t anchor,next_due;uint32_t delay;
 uint8_t repeats,index,decay,active; } note_fx_echo_state_t;
...
_Static_assert(sizeof(note_fx_echo_state_t)==64U,"Note FX Echo-state budget");
...
static SEQ_STATE_SDRAM note_fx_held_pitch_t
    g_held[NOTE_FX_SLOT_COUNT][SEQ_PRODUCT_HELD_STATE_CAPACITY];
...
static void refresh_work(uint8_t t,uint8_t s){const uint64_t b=slot_bit(t,s);if(model_is_generator(g_slot[t][s].model)&&g_slot[t][s].held_count)g_work_slot_mask|=b;else g_work_slot_mask&=~b;}
...
static note_fx_held_pitch_t*held_at(uint8_t slot,uint16_t lane,uint8_t branch){return &g_held[slot][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch];}
static void held_remove(note_fx_slot_runtime_t*r,note_fx_held_pitch_t*h){if(!h->source_token)return;memset(h,0,sizeof(*h));if(r->held_count)--r->held_count;}
static note_event_result_t held_ingest(uint8_t slot,note_fx_slot_runtime_t*r,const note_event_t*e){const uint16_t lane=product_lane(e);const uint8_t branch=note_event_branch(e);if(lane>=SEQ_PRODUCT_MAX_EMITTING_VOICES||branch>=SEQ_PRODUCT_HARMONY_FANOUT_MAX)return NOTE_EVENT_RESULT_REJECTED_DESTINATION;note_fx_held_pitch_t*h=held_at(slot,lane,branch);if(e->kind==NOTE_EVENT_KIND_OFF){if(h->source_token==e->source_id&&h->generation==e->source_generation)held_remove(r,h);return NOTE_EVENT_RESULT_ACCEPTED;}if(!h->source_token)++r->held_count;*h=(note_fx_held_pitch_t){.source_token=e->source_id,.generation=e->source_generation,.group_id=e->group_id,.lifetime_end=(e->duration_samples==NOTE_EVENT_DURATION_OPEN)?0U:(uint32_t)(e->sample_abs+e->duration_samples),.note=e->note,.velocity=e->velocity,.dependency_mask=e->dependency_mask,.destination_id=e->destination_id};return NOTE_EVENT_RESULT_ACCEPTED;}
static void held_expire(uint8_t t,uint8_t s,note_fx_slot_runtime_t*r,uint64_t sample){for(uint16_t lane=0;lane<SEQ_PRODUCT_MAX_EMITTING_VOICES;++lane)if(product_track(lane)==t)for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){note_fx_held_pitch_t*h=held_at(s,lane,branch);if(h->source_token&&held_deadline_reached(h,sample))held_remove(r,h);}}
...
static uint8_t arp_select(note_fx_slot_runtime_t*r,uint32_t step,uint8_t track,uint8_t slot,note_fx_held_pitch_t**oh,uint8_t*on){if(!r->held_count||!oh||!on)return 0;const note_fx_arp_style_t style=(note_fx_arp_style_t)r->p2;uint8_t rank;if(style==NOTE_FX_ARP_RANDOM){rank=(uint8_t)(mix32(step^((uint32_t)track<<24)^((uint32_t)slot<<16)^UINT32_C(0x9E3779B9))%r->held_count);}else if(style==NOTE_FX_ARP_UP_DOWN&&r->held_count>1U){const uint8_t cycle=(uint8_t)(2U*r->held_count-2U),pos=(uint8_t)(step%cycle);rank=pos<r->held_count?pos:(uint8_t)(cycle-pos);}else{rank=(uint8_t)(step%r->held_count);if(style==NOTE_FX_ARP_DOWN)rank=(uint8_t)(r->held_count-1U-rank);}note_fx_held_pitch_t*h=held_rank(track,slot,r,rank,(uint8_t)(style!=NOTE_FX_ARP_ORDER));if(!h)return 0;const uint8_t range=r->p3>=1&&r->p3<=4?r->p3:1;const uint32_t cycle=style==NOTE_FX_ARP_UP_DOWN&&r->held_count>1?(uint32_t)(2U*r->held_count-2U):r->held_count;const uint16_t raised=(uint16_t)h->note+(uint16_t)(12U*((step/cycle)%range));*on=raised<128U?(uint8_t)raised:h->note;*oh=h;return 1;}
static note_event_result_t emit_generated(uint8_t t,uint8_t s,const note_fx_held_pitch_t*h,uint8_t n,uint64_t sample,uint64_t duration,note_fx_emit_fn emit,void*ctx){if(!emit)return NOTE_EVENT_RESULT_ACCEPTED;const uint32_t token=next_token();const uint8_t mask=(uint8_t)(h->dependency_mask|(uint8_t)(1U<<s));const uint32_t bounded_duration=held_remaining(h,sample,duration>UINT32_MAX?UINT32_MAX:(uint32_t)duration);const uint16_t lane=(uint16_t)((h-&g_held[s][0])/SEQ_PRODUCT_HARMONY_FANOUT_MAX);const note_event_t e={.sample_abs=sample,.duration_samples=bounded_duration,.source_id=h->source_token,.intent_id=token,.source_generation=h->generation,.group_id=h->group_id?h->group_id:token,.dependency_versions=selected_versions(g_slot[t][s].dependency_versions,mask),.track=t,.destination_id=h->destination_id,.note=n,.velocity=h->velocity,.kind=NOTE_EVENT_KIND_ON,.provenance=NOTE_EVENT_SOURCE_FX,.stage=(uint8_t)(s+1U),.flags=NOTE_EVENT_FLAG_GENERATED,.temporal_index=lane_temporal(t,lane),.dependency_mask=mask};return emit(&e,ctx);}
...
{for(uint16_t i=0U;i<SEQ_PRODUCT_ECHO_STATE_CAPACITY;++i){note_fx_echo_state_t*x=&g_echo[i];
  while(x->active&&x->next_due<end){const uint64_t due=x->next_due;note_event_t e=x->seed;
   ++x->index;e.sample_abs=due;e.occurrence_id=child_id(e.occurrence_id,
       (uint8_t)(e.stage?e.stage-1U:0U),0U,x->index);
   ...
   if(e.duration_samples==NOTE_EVENT_DURATION_OPEN)e.duration_samples=x->delay;
...
void note_fx_engine_init(void){memset(&g_seq_context,0,sizeof(g_seq_context));memset(g_echo,0,sizeof(g_echo));memset(g_held,0,sizeof(g_held));memset(&g_echo_diag,0,sizeof(g_echo_diag));g_seq_context.samples_per_step_q16=UINT32_C(65536);}
...
void note_fx_engine_forget_dependency(uint8_t t,uint8_t owner){if(t>=NOTE_FX_TRACK_COUNT||owner>=NOTE_FX_SLOT_COUNT)return;const uint8_t bit=(uint8_t)(1U<<owner);for(uint8_t s=0;s<NOTE_FX_SLOT_COUNT;++s)for(uint16_t lane=0;lane<SEQ_PRODUCT_MAX_EMITTING_VOICES;++lane)if(product_track(lane)==t)for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){note_fx_held_pitch_t*h=held_at(s,lane,branch);if((h->dependency_mask&bit)!=0U)held_remove(&g_slot[t][s],h);}for(uint8_t s=0;s<NOTE_FX_SLOT_COUNT;++s)refresh_work(t,s);}
void note_fx_engine_forget_causal_sources_from_slot(uint8_t t,uint8_t first,const uint32_t*ids,uint16_t count){if(t>=NOTE_FX_TRACK_COUNT||first>=NOTE_FX_SLOT_COUNT||!ids)return;for(uint8_t s=first;s<NOTE_FX_SLOT_COUNT;++s)for(uint16_t lane=0;lane<SEQ_PRODUCT_MAX_EMITTING_VOICES;++lane)if(product_track(lane)==t)for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch){note_fx_held_pitch_t*h=held_at(s,lane,branch);for(uint16_t j=0;j<count;++j)if(h->source_token==ids[j]){held_remove(&g_slot[t][s],h);break;}}for(uint8_t s=first;s<NOTE_FX_SLOT_COUNT;++s)refresh_work(t,s);}
note_event_result_t note_fx_engine_cleanup(uint8_t t){if(t>=NOTE_FX_TRACK_COUNT)return NOTE_EVENT_RESULT_DROPPED_POLICY;for(uint8_t s=0;s<NOTE_FX_SLOT_COUNT;++s)held_clear_owner(t,s,&g_slot[t][s]);return NOTE_EVENT_RESULT_ACCEPTED;}
