...
static CONTROL_M4_SRAM2 note_fx_groove_lifetime_t
    g_groove_lifetime[NOTE_FX_SLOT_COUNT][SEQ_PRODUCT_HELD_STATE_CAPACITY];
...
static note_event_result_t direct(uint8_t slot,note_fx_slot_runtime_t*r,const note_event_t*e,note_event_t*out,uint8_t cap,uint8_t*count){const uint8_t stage=(uint8_t)(slot+1U);
 ...
 if(r->model==NOTE_FX_MODEL_GROOVE){note_event_t x=*e;const uint8_t phase=groove_phase(e->track,e->sample_abs);const note_fx_groove_template_t*t=&g_groove[r->p1%NOTE_FX_GROOVE_TYPE_COUNT];int32_t off=(int32_t)(((int64_t)t->timing[phase]*(int64_t)step_samples()*r->p2)/(96LL*100LL));const uint16_t lane=product_lane(e);const uint8_t branch=note_event_branch(e);note_fx_groove_lifetime_t*l=(lane<SEQ_PRODUCT_MAX_EMITTING_VOICES&&branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX)?&g_groove_lifetime[slot][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch]:NULL;if(source_is_live(e->source_id)&&l){if(e->kind==NOTE_EVENT_KIND_ON){l->source_id=e->source_id;l->generation=e->source_generation;l->offset=off;}else if(l->source_id==e->source_id&&l->generation==e->source_generation){off=l->offset;memset(l,0,sizeof(*l));}}x.sample_abs=sample_add_offset(x.sample_abs,off);if(source_is_live(e->source_id)&&x.sample_abs<e->sample_abs)x.sample_abs=e->sample_abs;if(x.kind==NOTE_EVENT_KIND_ON){int32_t v=x.velocity+((int32_t)t->velocity[phase]*x.velocity*r->p3)/10000;if(v<1)v=1;if(v>127)v=127;x.velocity=(uint8_t)v;}return append(out,cap,count,&x,stage)?NOTE_EVENT_RESULT_ACCEPTED:NOTE_EVENT_RESULT_REJECTED_CAPACITY;}
...
static void groove_clear_owner(uint8_t t,uint8_t s){for(uint16_t lane=0;lane<SEQ_PRODUCT_MAX_EMITTING_VOICES;++lane)if(product_track(lane)==t)for(uint8_t branch=0;branch<SEQ_PRODUCT_HARMONY_FANOUT_MAX;++branch)memset(&g_groove_lifetime[s][lane*SEQ_PRODUCT_HARMONY_FANOUT_MAX+branch],0,sizeof(note_fx_groove_lifetime_t));}
