...
static uint8_t capture_product_state(uint8_t entity,persist_control_entity_t*out)
{
    ...
    if (!(family==TRACK_FAMILY_SYNTH&&type==TRACK_TYPE_FM))
    {
        out->tone_present=1U;
...
persist_codec_result_t persistent_pattern_control_validate(const persist_control_pattern_t*p)
{
    ...
    for(uint8_t e=0U;e<PERSIST_CONTROL_ENTITY_COUNT;++e){if(persist_key_family_from_disk(p->entities[e].family,&cfg[e].family)==0U||persist_key_type_from_disk(p->entities[e].type,&cfg[e].type)==0U)return PERSIST_CODEC_INVALID_ENTITY;if(e<TRACK_COUNT){if((cfg[e].family==TRACK_FAMILY_EXTERNAL)&&(cfg[e].type==TRACK_TYPE_EXTERNAL)){if(persist_key_input_from_disk(p->entities[e].input_key,&inputs[e])==0U)return PERSIST_CODEC_INVALID_ENTITY;}else{if(p->entities[e].input_key!=PERSIST_INPUT_NONE)return PERSIST_CODEC_INVALID_ENTITY;inputs[e]=ENTITY_AUDIO_SOURCE_LINE;}}}
    for(uint8_t e=0U;e<PERSIST_CONTROL_ENTITY_COUNT;++e){persist_entity_caps_t caps;polyphony_control_state_t polyphony;audio_fx_control_state_t audio_fx;if(persist_entity_caps_resolve(active,e,&caps)==0U)return PERSIST_CODEC_INVALID_ENTITY;if(cfg[e].family==TRACK_FAMILY_OFF){if(cfg[e].type!=TRACK_TYPE_NONE)return PERSIST_CODEC_INVALID_ENTITY;}else if(track_catalog_type_is_valid_for_family(cfg[e].family,cfg[e].type)==false)return PERSIST_CODEC_INVALID_ENTITY;if(caps.active==0U&&cfg[e].family!=TRACK_FAMILY_OFF&&track_catalog_family_is_engine(cfg[e].family)==false)return PERSIST_CODEC_INVALID_ENTITY;if(caps.active!=0U&&cfg[e].family!=TRACK_FAMILY_OFF&&track_catalog_type_is_available(e,cfg[e].family,cfg[e].type,cfg)==false)return PERSIST_CODEC_INVALID_ENTITY;if(caps.active==0U)continue;if(cfg[e].family==TRACK_FAMILY_SYNTH&&cfg[e].type==TRACK_TYPE_FM){if(p->entities[e].fm_present==0U||fm_control_state_validate(&p->entities[e].fm)==0U)return PERSIST_CODEC_INVALID_ENTITY;}else if(p->entities[e].tone_present==0U||tone_program_control_validate(&p->entities[e].tone,track_runtime_type_from_ui(cfg[e].type))==0U)return PERSIST_CODEC_INVALID_ENTITY;if(param_filter_control_validate(&p->entities[e].filter)==0U||vca_control_state_validate(&p->entities[e].vca)==0U||mixer_control_state_validate(&p->entities[e].mixer)==0U||audio_fx_control_state_validate(&p->entities[e].audio_fx)==0U||polyphony_control_prepare(&p->entities[e].polyphony,&polyphony)==0U||audio_fx_control_state_prepare_for_polyphony(e,&p->entities[e].audio_fx,polyphony.voice_count,&audio_fx)==0U)return PERSIST_CODEC_INVALID_ENTITY;}
    for(uint8_t e=0U;e<PERSIST_CONTROL_ENTITY_COUNT;++e){const persist_control_entity_t*x=&p->entities[e];if(x->modulation_present==0U)continue;mod_lfo_control_bank_t lfos,prepared_lfos;mod_env3_control_state_t env,prepared_env;for(uint8_t i=0U;i<PERSIST_CONTROL_MOD_LFO_COUNT;++i){mod_lfo_shape_t shape;mod_lfo_trig_mode_t trigger;if(persist_key_lfo_shape_from_disk(x->modulation.lfos[i].shape_key,&shape)==0U||persist_key_lfo_trigger_from_disk(x->modulation.lfos[i].trigger_key,&trigger)==0U)return PERSIST_CODEC_INVALID_MODULATION;lfos.lfo[i]=(mod_lfo_control_value_t){x->modulation.lfos[i].rate,(float)shape,(float)trigger,x->modulation.lfos[i].phase_offset};}env=(mod_env3_control_state_t){x->modulation.envelope.attack,x->modulation.envelope.decay,x->modulation.envelope.sustain,x->modulation.envelope.release,(float)x->modulation.envelope.retrigger_hard};if(mod_lfo_v1_prepare_bank(&lfos,&prepared_lfos)==0U||mod_env3_control_prepare(&env,&prepared_env)==0U)return PERSIST_CODEC_INVALID_MODULATION;}
    if(track_input_ownership_validate_bulk(cfg,inputs)==0U
...
{polyphony_control_state_t pp;audio_fx_control_state_t pa;live_parameter_audio_bulk_t bulk={.capture_tick=0U};if(!polyphony_control_prepare(polyphony,&pp)||!audio_fx_control_state_prepare_for_polyphony(entity,audio_fx,pp.voice_count,&pa)||!polyphony_control_bulk_add(entity,&pp,&bulk)||!audio_fx_control_state_bulk_add_prepared(entity,&pa,&bulk)||!live_parameter_audio_publication_submit_bulk_now(&bulk))return 0U;return polyphony_control_install_prepared(entity,&pp)&&audio_fx_control_state_install_prepared(entity,&pa);}
    ...
    for (uint8_t entity = 0U; entity < BRICK_ENTITY_CAPACITY; ++entity)
    {
        ...
        if (entity < TRACK_COUNT)
        {
            if ((family == TRACK_FAMILY_EXTERNAL)
                    && (type == TRACK_TYPE_EXTERNAL))
            {
                if (persist_key_input_from_disk(
                        pattern->entities[entity].input_key,
                        &inputs[entity]) == 0U)
                    return PERSIST_CODEC_INVALID_ENTITY;
            }
            else
            {
                if (pattern->entities[entity].input_key != PERSIST_INPUT_NONE)
                    return PERSIST_CODEC_INVALID_ENTITY;
                inputs[entity] = ENTITY_AUDIO_SOURCE_LINE;
            }
        }
    }
