...
static uint8_t apply_core(void*ctx,const persist_control_project_t*p){(void)ctx;if(!project_control_validate_assets(p->assets,p->asset_count))return 0U;pattern_control_bank_clear_project();persistent_project_apply_context_t c={0U,PERSIST_CODEC_OK};return persistent_project_control_apply_progressive_core(&c,p);}
