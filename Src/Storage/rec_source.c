...
#define REC_SOURCE_PROMOTION_JOURNAL "0:/REC/REC_PROMOTE.JRN"
...
typedef struct
{
    ...
    audio_recorder_waveform_summary_t waveform;
...
static uint8_t write_promotion_journal(const rec_source_promotion_journal_t *journal)
{
    FIL file;
    UINT written = 0U;
    (void)f_unlink(REC_SOURCE_PROMOTION_JOURNAL);
    if (f_open(&file, REC_SOURCE_PROMOTION_JOURNAL,
...
static void recover_promotion(void)
{
    ...
    if (f_open(&file, REC_SOURCE_PROMOTION_JOURNAL, FA_READ) != FR_OK)
    { g_rec_source_recovery_pending = 0U; return; }
    const FRESULT read_result = f_read(&file, &journal, sizeof(journal), &read);
    (void)f_close(&file);
    if ((read_result != FR_OK) || (read != sizeof(journal))
            || (journal.magic != REC_SOURCE_PROMOTION_MAGIC))
    { g_rec_source_recovery_pending = 0U; return; } /* preserve every path */
    ...
    if ((old_exists != 0U) && (new_exists == 0U))
        (void)f_unlink(REC_SOURCE_PROMOTION_JOURNAL); /* rollback truth */
                                    ...
                                    const audio_recorder_storage_map_copy_t *map)
{
    ...
    if (audio_recorder_capture_audio_waveform_export(&building->waveform) == 0U)
        return 0U;
...
uint8_t rec_source_promote_current(const char *persistent_path)
{
    ...
    if ((copy_path(journal.old_path, current->path) == 0U)
            ...
            || (write_promotion_journal(&journal) == 0U)) return 0U;
    if (f_rename(current->path, persistent_path) != FR_OK) return 0U;
