...
static uint8_t codec_type_valid(uint32_t key)
{
    ...
    case PERSIST_TYPE_STREAM_SAMPLE:case PERSIST_TYPE_MULTI_SAMPLE:
...
static void codec_sequence(codec_io_t *io, persist_control_entity_t *entity,uint8_t group_active)
{
    ...
    codec_u8(io,&s->timing.random);codec_i8(io,&s->timing.velocity);
    codec_u8(io,&s->timing.global);
    if(io->mode==CODEC_READ){s->timing.groove=SEQ_GROOVE_NONE;
        s->timing.groove_flags=0U;}
...
static void codec_entity(codec_io_t *io, persist_control_entity_t *e,uint8_t group_active)
{
    ...
    codec_bytes(io,(uint8_t *)&e->note_fx,PERSIST_CONTROL_NOTE_FX_BYTES);
...
{
    if (id == PARAM_MIDI_FX_GENERATOR_P4) return 0U;
...
static uint8_t codec_note_fx_state_valid(const note_fx_chain_state_t *state,
                                         uint8_t owner)
{
    if (state == NULL) return 0U;
    const uint8_t *const values = (const uint8_t *)state;
    if (owner == 0U)
    {
        for (uint8_t i = 0U; i < PERSIST_CONTROL_NOTE_FX_BYTES; ++i)
            if (values[i] != 0U) return 0U;
        return 1U;
    }
    for (uint8_t i = 0U; i < PERSIST_CONTROL_NOTE_FX_BYTES; ++i)
    {
        const param_id_t id = (param_id_t)(PARAM_MIDI_FX_GENERATOR_P1 + i);
        if (((float)values[i] < param_registry[id].min)
                || ((float)values[i] > param_registry[id].max)) return 0U;
    }
    return 1U;
}
...
persist_codec_result_t persist_codec_validate_pattern(const persist_control_pattern_t *p)
{
    ...
    { const persist_control_entity_t *x=&p->entities[e];persist_entity_caps_t caps;uint8_t input=0U;if((x->entity_id!=e)||(persist_entity_caps_resolve(group_active,e,&caps)==0U)||(caps.persistable==0U)||(codec_family_valid(x->family)==0U)||(codec_type_valid(x->type)==0U)||(codec_entity_assets_valid(x)==0U)||(x->midi_channel<1U)||(x->midi_channel>16U)||(codec_midi_source_valid(x->midi_source_key)==0U)||(persist_key_input_from_disk(x->input_key,&input)==0U)||(x->polyphony.voice_count<1U)||(x->polyphony.voice_count>8U)||(x->audio_fx.config.filter_position>=3U)||(x->audio_fx.config.order>=2U)||(x->audio_fx.config.spatial_mode[0]>=4U)||(x->audio_fx.config.spatial_mode[1]>=4U)||(x->muted>1U)||(x->fm_present>1U)||(x->tone_present>1U)||(x->fm_present&&x->tone_present)||((caps.input_owner==0U)&&(input!=0U))||((x->family==PERSIST_FAMILY_OFF)&&(x->muted!=0U)))return PERSIST_CODEC_INVALID_ENTITY;
      if((caps.active==0U)&&((x->asset_count!=0U)||(x->fm_present!=0U)||(x->tone_present!=0U)||(x->muted!=0U)||(codec_note_fx_state_valid(&x->note_fx,0U)==0U)||(x->modulation_present!=0U)))return PERSIST_CODEC_INVALID_ENTITY;
...
{ if(p==NULL)return PERSIST_CODEC_INVALID_ARGUMENT;if((p->name_length>PERSIST_CONTROL_PATCH_NAME_BYTES)||(codec_family_valid(p->family)==0U)||(codec_type_valid(p->type)==0U)||(p->asset_count>PERSIST_CONTROL_TRACK_ASSET_COUNT)||(p->fm_present>1U)||(p->tone_present>1U)||(p->modulation_present>1U)||(p->fm_present&&p->tone_present)||(p->polyphony.voice_count<1U)||(p->polyphony.voice_count>8U))return PERSIST_CODEC_CAPACITY_EXCEEDED;for(uint8_t i=0U;i<p->asset_count;++i)if(codec_validate_asset_ref(&p->assets[i])!=PERSIST_CODEC_OK)return PERSIST_CODEC_INVALID_ASSET;return PERSIST_CODEC_OK; }
...
static void codec_section_version(codec_io_t *io,uint16_t type,uint16_t version,codec_body_fn fn,void *object)
{
    ...
    uint32_t length=measure.count;codec_u16(io,&type);codec_u16(io,&version);codec_u32(io,&length);
...
static void codec_section(codec_io_t *io,uint16_t type,codec_body_fn fn,void *object){codec_section_version(io,type,1U,fn,object);}
...
static void codec_project_payload(codec_io_t*io,void*v){codec_section_version(io,SECTION_PROJECT_CORE,2U,codec_project_core_encode,v);codec_section(io,SECTION_PROJECT_ASSETS,codec_project_assets_encode,v);codec_section(io,SECTION_PROJECT_MACROS,codec_project_macros_encode,v);codec_section(io,SECTION_PROJECT_BANK,codec_project_bank_encode,v);}
    ...
    uint32_t *out_bytes)
{
    if ((macros == NULL) || (sink == NULL) || (sink->write == NULL))
        return PERSIST_CODEC_INVALID_ARGUMENT;
    persist_codec_result_t result = persist_codec_validate_macros(macros);
    if (result != PERSIST_CODEC_OK) return result;
    codec_io_t io = {.mode=CODEC_WRITE,.sink=sink,
                     .limit=PERSIST_CODEC_MAX_DOCUMENT_BYTES,
                     .result=PERSIST_CODEC_OK};
    ...
    uint8_t out_header[8])
{
    ...
    out_header[2] = (section == PERSIST_CODEC_PROJECT_SECTION_CORE) ? 2U : 1U;
    out_header[3] = 0U;
