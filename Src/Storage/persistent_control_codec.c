...
static void codec_sequence(codec_io_t *io, persist_control_entity_t *entity,uint8_t group_active)
{
    ...
    for(uint8_t step=0U;step<PERSIST_CONTROL_STEP_COUNT;++step)
    {
        ...
        for(uint8_t i=0U;i<st->play_count;++i)
        {
            ...
            codec_bytes(io,(uint8_t *)&p->microtiming,1U);codec_u8(io,&p->present);
        }
...
static void codec_modulation(codec_io_t *io, persist_control_modulation_t *m)
{
    ...
    codec_f32(io,&m->envelope.attack);codec_f32(io,&m->envelope.decay);codec_f32(io,&m->envelope.sustain);codec_f32(io,&m->envelope.release);
...
persist_codec_result_t persist_codec_decode_project(const persist_codec_source_t *s,persist_codec_project_staging_t *st,const persist_codec_pattern_consumer_t *patterns){if((st==NULL)||(patterns==NULL)||(patterns->put==NULL)||(patterns->commit==NULL)||(patterns->abort==NULL))return PERSIST_CODEC_INVALID_ARGUMENT;memset(st,0,sizeof(*st));codec_io_t io;uint32_t crc;persist_codec_result_t r=codec_decode_begin(s,PERSIST_CODEC_DOCUMENT_PROJECT,4U,&io,&crc);if(r!=PERSIST_CODEC_OK)return r;project_ctx_t c={.p=&st->project,.consumer=*patterns,.record=&st->pattern_record};codec_expect_section(&io,SECTION_PROJECT_CORE,codec_project_core,&c);codec_expect_section(&io,SECTION_PROJECT_ASSETS,codec_project_assets,&c);codec_expect_section(&io,SECTION_PROJECT_MACROS,codec_project_macros,&c);codec_expect_section(&io,SECTION_PROJECT_BANK,codec_project_bank,&c);r=codec_decode_end(&io,crc);if(r==PERSIST_CODEC_OK)r=persist_codec_validate_project(&st->project);if((r==PERSIST_CODEC_OK)&&(patterns->commit(patterns->context)==0U))r=PERSIST_CODEC_IO_ERROR;if(r!=PERSIST_CODEC_OK)patterns->abort(patterns->context);return r;}
