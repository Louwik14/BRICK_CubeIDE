...
void pattern_control_bank_init(void){memset(g_present,0,sizeof(g_present));g_active_set=INVALID_SET;g_staging_set=INVALID_SET;g_generation=0U;if(!acquire())return;(void)f_mkdir("0:/PATTERN");(void)f_mkdir("0:/PATTERN/S0");(void)f_mkdir("0:/PATTERN/S1");uint32_t g0=0U,g1=0U;uint8_t v0=read_commit(0U,&g0),v1=read_commit(1U,&g1);if(v0&&v1&&g0!=g1){g_active_set=((int32_t)(g1-g0)>0)?1U:0U;g_generation=(g_active_set==1U)?g1:g0;}else if(v0&&!v1){g_active_set=0U;g_generation=g0;}else if(v1&&!v0){g_active_set=1U;g_generation=g1;}else if(!v0&&!v1){uint8_t legacy;if(read_legacy_marker(&legacy))g_active_set=legacy;else{g_active_set=0U;migrate_initial_set();}uint8_t bitmap[32];scan_bitmap(g_active_set,bitmap);g_generation=1U;if(!write_commit(g_active_set,g_generation,bitmap))g_active_set=INVALID_SET;}if(g_active_set==INVALID_SET){clear_set(0U);clear_set(1U);g_active_set=0U;uint8_t blank[32]={0};g_generation=1U;(void)write_commit(g_active_set,g_generation,blank);}(void)f_unlink("0:/PATTERN/ACTIVE.BIN");(void)f_unlink("0:/PATTERN/ACTIVE.TMP");scan_active();sd_access_gate_release(SD_ACCESS_CLIENT_PATTERN);}
