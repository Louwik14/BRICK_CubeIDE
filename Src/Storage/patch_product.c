...
patch_product_result_t patch_product_save(uint8_t e,uint16_t*out){uint16_t s=(g_current<PATCH_PRODUCT_SLOT_COUNT&&!g_invalid[g_current])?g_current:patch_product_first_empty();if(s==PATCH_PRODUCT_INVALID_SLOT)return PATCH_PRODUCT_NO_SLOT;persist_control_patch_t p;char generated[33];const char*name=(g_present[s]&&g_meta[s].name[0])?g_meta[s].name:generated;if(name==generated)(void)snprintf(generated,sizeof(generated),"T%02u %s",(unsigned)(e+1U),ui_get_track_family_short_name(track_state_get_family(e)));if(persistent_patch_control_capture(e,name,&p)!=PERSIST_CODEC_OK)return PATCH_PRODUCT_INVALID;if(!store(s,&p))return PATCH_PRODUCT_IO_ERROR;g_current=s;if(out)*out=s;return PATCH_PRODUCT_OK;}
