...
static const param_id_t g_seq_param_midi_fx_param_to_id[SEQ_PARAM_MIDI_FX_PARAM_COUNT] = {
    ...
    PARAM_MIDI_FX_GENERATOR_P4,
    PARAM_MIDI_FX_VOICER_P1,
    PARAM_MIDI_FX_VOICER_P2,
    PARAM_MIDI_FX_VOICER_P3,
    PARAM_MIDI_FX_VOICER_P4,
    PARAM_MIDI_FX_SCALER_P1,
    PARAM_MIDI_FX_SCALER_P2,
    PARAM_MIDI_FX_SCALER_P3,
    PARAM_MIDI_FX_SCALER_P4,
    PARAM_MIDI_FX_TRIG_P1,
    PARAM_MIDI_FX_TRIG_P2,
    PARAM_MIDI_FX_TRIG_P3,
    PARAM_MIDI_FX_TRIG_P4,
};
...
static const seq_param_compact_map_t g_seq_param_param_to_slot[PARAM_COUNT] = {
    ...
    [PARAM_MIDI_FX_GENERATOR_P1] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 0U },
    [PARAM_MIDI_FX_GENERATOR_P2] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 1U },
    [PARAM_MIDI_FX_GENERATOR_P3] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 2U },
    [PARAM_MIDI_FX_GENERATOR_P4] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 3U },
    [PARAM_MIDI_FX_VOICER_P1] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 4U },
    [PARAM_MIDI_FX_VOICER_P2] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 5U },
    [PARAM_MIDI_FX_VOICER_P3] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 6U },
    [PARAM_MIDI_FX_VOICER_P4] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 7U },
    [PARAM_MIDI_FX_SCALER_P1] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 8U },
    [PARAM_MIDI_FX_SCALER_P2] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 9U },
    [PARAM_MIDI_FX_SCALER_P3] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 10U },
    [PARAM_MIDI_FX_SCALER_P4] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 11U },
    [PARAM_MIDI_FX_TRIG_P1] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 12U },
    [PARAM_MIDI_FX_TRIG_P2] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 13U },
    [PARAM_MIDI_FX_TRIG_P3] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 14U },
    [PARAM_MIDI_FX_TRIG_P4] = { (uint8_t)SEQ_PLOCK_SET_MIDI_FX, 15U }
...
static seq_param_slot_state_t *seq_param_iface_state_at(seq_track_id_t track, uint8_t set_id, seq_param_slot_t param_slot)
{
    ...
    if (set_id == (uint8_t)SEQ_PLOCK_SET_TONE)
    {
        ...
        if ((seq_param_iface_resolve_runtime_tone_type(track, &tone_type) == 0U)
                || (tone_param_codec_slot_to_param(tone_type, param_slot, &tone_param) == 0U))
        {
            return 0;
        }
    }
...
static uint8_t seq_param_iface_param_matches_set_domain(seq_track_id_t track,
                                                        ...
                                                        param_id_t param)
{
    return seq_param_iface_param_matches_set_domain_context(
        (seq_param_iface_is_group_master(track) != 0U), set_id, param);
}
...
{
    if ((out_param_slot == NULL) || (param_id >= PARAM_COUNT)
        || (set_id >= (uint8_t)SEQ_PLOCK_SET_COUNT)
        || (seq_param_iface_param_matches_set_domain_context(
                group_master, set_id, param_id) == 0U))
        return 0U;

    if (set_id == (uint8_t)SEQ_PLOCK_SET_TONE)
    {
        uint8_t slot = 0U;
        if (tone_param_codec_param_to_slot(
                (track_runtime_type_t)runtime_type, param_id, &slot) == 0U)
            return 0U;
        *out_param_slot = (seq_param_slot_t)slot;
        return 1U;
    }
    if (set_id == (uint8_t)SEQ_PLOCK_SET_FM_OPERATOR)
    {
        if ((runtime_type != (uint8_t)TRACK_RUNTIME_TYPE_FM)
            || (param_id < PARAM_FM_OPERATOR_FIRST)
            || (param_id > PARAM_FM_OPERATOR_LAST))
            return 0U;
                                                         ...
                                                         seq_param_slot_t param_slot)
{
    ...
    if (seq_param_iface_is_group_master(track) != 0U)
    {
        ...
        return (uint8_t)((status == TRACK_RUNTIME_PARAM_ALLOWED)
                || (status == TRACK_RUNTIME_PARAM_GLOBAL_ALLOWED));
    }
    return 1U;
}
                                      ...
                                      param_id_t *out_param_id)
{
    ...
    if (set_id == (uint8_t)SEQ_PLOCK_SET_TONE)
    {
        ...
        param_id_t tone_param = PARAM_COUNT;
        if (tone_param_codec_slot_to_param(tone_type, param_slot, &tone_param) == 0U)
        {
            return 0U;
        }
        if ((tone_param >= PARAM_COUNT) || (seq_param_iface_param_matches_set_domain(track, set_id, tone_param) == 0U))
        {
            return 0U;
        }
    ...
    if (set_id == (uint8_t)SEQ_PLOCK_SET_FM_OPERATOR)
    {
        ...
        if ((param_slot >= SEQ_PARAM_FM_OPERATOR_SLOT_COUNT)
                || (seq_param_iface_param_matches_set_domain(track, set_id, operator_param) == 0U))
        {
            return 0U;
        }
    ...
    if ((param >= PARAM_COUNT)
            || (seq_param_iface_param_matches_set_domain(track, set_id, param) == 0U))
    {
        return 0U;
    }
                                      ...
                                      seq_param_slot_t *out_param_slot)
{
    ...
    if (seq_param_iface_param_matches_set_domain(track, set_id, param_id) == 0U)
    {
        return 0U;
    }
    ...
    if (set_id == (uint8_t)SEQ_PLOCK_SET_TONE)
    {
        ...
        if (tone_param_codec_param_to_slot(tone_type, param_id, &tone_slot) == 0U)
        {
            return 0U;
        }
    ...
    param_id_t *out_param_id)
{
    ...
    if (set_id == (uint8_t)SEQ_PLOCK_SET_TONE)
    {
        if (tone_param_codec_slot_to_param((track_runtime_type_t)runtime_type,
                                             param_slot,
                                             &param) == 0U)
        {
            return 0U;
