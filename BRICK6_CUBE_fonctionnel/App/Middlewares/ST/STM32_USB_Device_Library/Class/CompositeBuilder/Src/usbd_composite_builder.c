...
uint8_t  USBD_CMPSIT_AddToConfDesc(USBD_HandleTypeDef *pdev)
{
  ...
  switch (pdev->tclasslist[pdev->classId].ClassType)
  {
    ...
    case CLASS_TYPE_MIDI:
      /* Setup Max packet sizes */
      pdev->tclasslist[pdev->classId].CurrPcktSze = MIDI_EPIN_SIZE;
      ...
      /* Assign endpoint numbers */
      pdev->tclasslist[pdev->classId].NumEps = 2U; /* EP IN, EP OUT */

      /* Set IN endpoint slot */
      iEp = pdev->tclasslist[pdev->classId].EpAdd[0];
      USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_BULK, MIDI_EPIN_SIZE);
