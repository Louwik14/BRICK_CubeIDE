...
- La résolution `UI track -> filter target runtime` est centralisée dans `track_runtime` (compat legacy conservée, pas de second routeur opérationnel).
- Politique refresh runtime: les checks/getters (`status`, résolution cible) restent lecture-only; l’invalidation est explicite (changement family/type) et les refresh sont déclenchés explicitement par les chemins appelants (UI/param/seq/mod) selon besoin.
...
- engines drum hébergés via un catalogue runtime dédié (pas de recyclage des types `Synth`)
