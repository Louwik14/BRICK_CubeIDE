...
static uint32_t project_sd_checksum(const uint8_t *data, uint32_t len)
{
    uint32_t crc = 5381UL;
...
uint8_t project_sd_bank_load_slot(uint8_t project_slot, ProjectSaveV1 *out_project, uint32_t *out_save_counter)
{
    ...
    if (project_sd_checksum((const uint8_t *)out_project, sizeof(*out_project)) != hdr.checksum)
    {
        goto done;
    }
...
uint8_t project_sd_bank_store_slot(uint8_t project_slot, const ProjectSaveV1 *project, uint32_t save_counter)
{
    ...
    hdr.checksum = project_sd_checksum((const uint8_t *)project, sizeof(*project));
