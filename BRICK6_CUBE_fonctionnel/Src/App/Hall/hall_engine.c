...
static uint8_t hall_button_apply_calibration(volatile hall_button_t *button,
                                           uint16_t min_value,
                                           uint16_t max_value)
{
    if (button == 0)
    ...
    button->min = min_value;
    button->max = max_value;
    hall_button_update_thresholds(button);

    return button->range_valid;
}
                                 ...
                                 const uint16_t *max_values)
{
    ...
    for (uint8_t i = 0U; i < HALL_KEY_COUNT; i++)
    {
        const uint8_t key_valid = hall_button_apply_calibration(&hall_button[i],
                                                                min_values[i],
                                                                max_values[i]);
