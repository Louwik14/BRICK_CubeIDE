...
#define SEQ_RUNTIME_MIDI_CHANNEL 0U
#define SEQ_RUNTIME_MIDI_DEST MIDI_DEST_USB
...
static uint8_t seq_runtime_track_supports_play(seq_track_id_t track)
{
    uint8_t set_id = 0U;
    seq_param8_t param8 = 0U;
    if (seq_param_iface_map_param(PARAM_SEQ_PLAY_V1_NOTE, &set_id, &param8) == 0U)
    {
        return 0U;
    }

    return seq_param_iface_is_param_supported(track, set_id, param8);
}
...
static void seq_runtime_play_events_service(void)
{
    ...
    while (i < g_seq_play_event_count)
    {
        ...
        const uint8_t channel = SEQ_RUNTIME_MIDI_CHANNEL;
        if (evt->type == (uint8_t)SEQ_PLAY_EVT_NOTE_ON)
        {
            midi_note_on(SEQ_RUNTIME_MIDI_DEST, channel, evt->note, evt->velocity);
        }
        else
        {
            midi_note_off(SEQ_RUNTIME_MIDI_DEST, channel, evt->note, 0U);
        }
...
{
    if ((seq_runtime_track_supports_play(track) == 0U)
        || (seq_model_get_trig(track, step) == 0U))
    {
        return;
    }
