...
void ui_tasklet_poll(void)
{
    ...
    else if(page == 1U)
    {
        ...
        if(d3 != 0)
        {
            hpf_hz = clampf(hpf_hz + ((float)d3 * 5.0f), 20.0f, 200.0f);
            control_router_set_param(CTRL_PARAM_BUS_COMP_HPF_HZ, hpf_hz);
        }
    }
    // -------- PAGE 2 --------
    ...
    else if(page == 1U)
    {
        ...
        snprintf(line3, sizeof(line3), "HPF       %d Hz", (int)hpf_hz);
    }
    else
    {
        ...
        snprintf(line2, sizeof(line2), "Threshold %.1f dB", (double)threshold_db);
        snprintf(line3, sizeof(line3), "Ratio     %.1f:1", (double)ratio);
    }
