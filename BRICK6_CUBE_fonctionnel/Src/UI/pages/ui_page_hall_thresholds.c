...
static void ui_page_hall_thresholds_handle_event(const ui_event_t *ev)
{
    ...
    const int16_t dir = (ev->value > 0) ? 1 : -1;

    if (ev->id == 0U)
    {
        const uint16_t current = hall_engine_get_threshold_ppm();
        const uint16_t next = ui_clamp_u16((int32_t)current + ((int32_t)dir * 5), 0U, 1000U);
    ...
    else if (ev->id == 2U)
    {
        ...
        const uint16_t next = ui_clamp_u16((int32_t)current + dir, 1U, 65534U);
        hall_engine_set_time_fast_dt(next);
    }
    else if (ev->id == 3U)
    {
        const uint16_t current = hall_engine_get_time_slow_dt();
        const uint16_t next = ui_clamp_u16((int32_t)current + dir, 2U, 65535U);
