...
constexpr uint32_t kReleaseSamples = 4u;  // 16..64, très cheap
...
extern "C" void fx_granular_process_block(float* in_l, float* in_r,
  ...
  const uint32_t offset_fixed = ((uint32_t)(g_state.stereo_offset * 512.0f)) << 16;
  ...
  for (uint32_t n = 0; n < frames; ++n) {
    ...
    for (uint32_t i = 0; i < kMaxGrains; ++i) {
      ...
      if (g.interp_toggle == 0) {
        g.lastL = read_interp_fixed(bufL, g.pos);

        uint32_t posR = g.pos + offset_fixed;
        if (posR >= limit) posR -= limit;
        g.lastR = read_interp_fixed(bufR, posR);
      }
      g.interp_toggle ^= 1;

      // ✅ smoothing low-cost du sample&hold (réduit les artefacts SRR)
      ...
      // ✅ release anti-pop sur les derniers samples
