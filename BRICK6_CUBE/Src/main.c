...
int main(void)
{
  ...
  if (!audio_init())
  {
    uart_log("audio init failed\r\n");
    Error_Handler();
  }
  if (!audio_start())
  {
    uart_log("audio start failed\r\n");
    Error_Handler();
  }
  ...
  while (1)
  {
    ...
    if ((now - last_log_tick) >= 1000U)
    {
      ...
      uint32_t audio_half = 0;
      uint32_t audio_full = 0;
      audio_get_stats(&audio_half, &audio_full);
      snprintf(log_buffer, sizeof(log_buffer),
               "SAI state=%lu err=0x%08lX half=%lu full=%lu\r\n",
               (unsigned long)hsai_BlockA1.State,
               (unsigned long)error,
               (unsigned long)audio_half,
               (unsigned long)audio_full);
      uart_log(log_buffer);
